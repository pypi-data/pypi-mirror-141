...
namespace chm {
	...
	public:
		...
		void push(const float dist, const uint id);
	...
	inline Heap<Comparator>::Heap(const Node& ep) {
		this->nodes.emplace_back(ep.dist, ep.id);
	}
	...
	inline void Heap<Comparator>::push(const Node& n) {
		this->push(n.dist, n.id);
	}

	template<class Comparator>
	inline void Heap<Comparator>::push(const float dist, const uint id) {
		this->nodes.emplace_back(dist, id);
