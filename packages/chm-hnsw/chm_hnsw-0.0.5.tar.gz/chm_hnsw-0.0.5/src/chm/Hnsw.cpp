...
namespace chm {
	void Hnsw::fillFarHeap(const uint currentID, const float* const query, const uint queryID) {
	...
	void Hnsw::push(const float* const data) {
	...
	FarHeap Hnsw::query(const float* const data, const uint k) {
	...
	void Hnsw::resetEp(const float* const query) {
		this->ep.dist = this->space.getDistance(query, this->entryID);
		this->ep.id = this->entryID;
	}

	void Hnsw::searchUpperLayer(const float* const query, const uint lc) {
		uint prev{};
	...
	void Hnsw::selectNeighborsHeuristic(const uint M) {
	...
	std::string Hnsw::getString() const {
		std::stringstream s;
		s << "Hnsw(efConstruction=" << this->cfg.efConstruction << ", mMax=" << this->cfg.mMax << ", distance=" << this->space.getName() << ')';
		return s.str();
	}
	...
	void Hnsw::push(const NumpyArray data) {
		size_t i = 0;
	...
	py::tuple Hnsw::query(const NumpyArray data, const uint k) {
		const DataInfo info(data, this->space.dim);
	...
	void Hnsw::setEfSearch(const uint efSearch) {
