...
def plantilla(proyecto=None, subproyecto=None, path_proyectos=None):
    ...
    if os.path.isdir(test_path):
        ...
        if os.path.isdir(path_user)==False:
            raise NameError(f"Error 404. Directorio {path_user} no encontrado, revisar nombre del proyecto")
    ...
    if os.path.isdir(path_user + fr'\scripts\{subproyecto}'):
        ...
        raise NameError(f"El directorio {path_user + fr'\scripts\{subproyecto}'} no existe. Por favor, crear una carpeta en scripts con el nombre del subproyecto.")
