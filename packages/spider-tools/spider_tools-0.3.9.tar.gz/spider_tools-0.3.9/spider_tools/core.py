...
class Auto_sinsert():
    def __init__(self, host='127.0.0.1', username='root', password='mysql', db='test',
        ...
        if pool_db:
            ...
            self.conn = pymysql.connect(host=self.host, user=self.username, passwd=self.password, db=self.db,port=port, charset='utf8')
            self.cursor = self.conn.cursor()
    ...
    def insert_data(self, item, table_name):
        ...
        if item and sql_conn and cursor:
            ...
            if not cursor and not sql_conn:
                ...
                red_print("数据库连接异常，未插入数据字段保存在 error_insert_data.txt")
            else:
                print("item is None")
        cursor.close()
        sql_conn.close()

    def insert_data_many(self, item_list, table_name,num=500):
        sql_conn = self.sql_pool.connection()
        cursor = sql_conn.cursor()
        AllDataList = []
        insert = None
