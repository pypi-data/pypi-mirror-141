...
def ci_checks(checks_data_file, input_ntuple, output_dir):
    ...
    hist_to_root(
        test_info["jobs"][0]["name"], check_results, f"{output_dir}/checks.root"
    )

    check_results = {test_info["jobs"][0]["name"]: check_results}
    checks_to_JSON(
        test_info["jobs"][0]["checks_data"],
        check_results,
