...
def validate_config(config_dict):
    ...
    if 'defaults' not in config_dict:
        raise InvalidConfigurationException('invalid configuration file: missing `defaults`')
    if 'environments' not in config_dict:
        raise InvalidConfigurationException('invalid configuration file: missing `environments`')
    if 'defaults' in config_dict['environments']:
        raise InvalidConfigurationException('invalid configuration file: illegal environment name `defaults`')
...
def read_config_file(config_path=None):
    ...
    config_path_or_default = config_path or '$HOME/.config/symbiont/python-sdk-config.json'
