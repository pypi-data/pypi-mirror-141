...
def ISE_ers_all(url, username, password):
    ...
    ISE_interal_users(url, username, password)
...
def ISE_active_sessions(url, username, password):
    try:
        ...
        activeSessionCount = requests.request("GET", f"{ url }:443/admin/API/mnt/Session/ActiveCount", headers=headers, auth=(username, password), verify=False)
        print(activeSessionCount)
        ...
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = active_session_template.render(activeSessionCount = activeSessionCount,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
    ...
    # -------------------------
                if loop_counter <= 3:
                    ...
                    with open("ISE Active Sessions Mind Map.md", "w") as fh:
                        fh.write(activeSessionCountJSON)               
                        fh.close()
