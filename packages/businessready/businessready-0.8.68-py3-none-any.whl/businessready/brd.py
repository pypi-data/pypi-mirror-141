...
def ISE_csr(url, username, password):
    try:
        ...
        if csrAllJSON is not None:
            ...
            loop_counter = 0
            csr = []
            if csrAllJSON['nextPage'] is not "null": 
                for href in csrAllJSON:
                    ...
                    csr.append(csrHrefJSON)          
            else:
                csr.append(csrAllJSON)
        # Render Templates
                for filetype in filetype_loop:
                    parsed_output = sponsor_guest_portal_template.render(csr = csr,filetype_loop=loop_counter)
                    loop_counter = loop_counter + 1
