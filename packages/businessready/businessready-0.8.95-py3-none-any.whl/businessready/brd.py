...
def ISE_license_connection_type(url, username, password):
    try:
        ...
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = licenseCon_template.render(licenseCon = licenseConJSON['response'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
    ...
    # -------------------------
                ...
                with open(f"ISE License Connection Type.json", "w") as fh:
                    json.dump(licenseConJSON['response'], fh, indent=4, sort_keys=True)
                    fh.close()                            
        return(licenseConJSON['response'])
    except Exception as e:
        logging.exception(e)
