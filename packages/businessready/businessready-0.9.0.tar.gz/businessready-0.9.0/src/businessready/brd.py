...
def ISE_license_register(url, username, password):
    try:
        ...
        licRegister = requests.request("GET", f"{ url }/api/v1/license/system/feature-to-tier-mapping", headers=headers, auth=(username, password), verify=False)
        licRegisterJSON = licRegister.json()

        # Pass to template 

        if licRegisterJSON is not None:
            licRegister_template = env.get_template('ISE_license_feature.j2')
            loop_counter = 0
