...
def ISE_deployment_node(url, username, password):
    try:
        ...
        if deploymentNodeAllJSON is not None:
            ...
            loop_counter = 0
            nodes = []
            if deploymentNodeAllJSON['nextPage'] is not None: 
                for href in certsAllJSON:
                    deploymentNodeHref = requests.request("GET", href['nextPage']['href'], headers=headers, auth=(username, password), verify=False)
                    deploymentNodeHrefJSON = deploymentNodeHref.json()
                    nodes.append(deploymentNodeHrefJSON['response'])          
            else:
                nodes.append(deploymentNodeAllJSON['response'])
        # Render Templates
                for filetype in filetype_loop:
                    parsed_output = deployment_node_template.render(nodes = nodes,filetype_loop=loop_counter)
                    loop_counter = loop_counter + 1
    ...
    # -------------------------
                    ...
                    with open(f"ISE Deployment Node.json", "w") as fh:
                        json.dump(nodes, fh, indent=4, sort_keys=True)
                        fh.close()                            
        return(nodes)
    except Exception as e:
        logging.exception(e)
