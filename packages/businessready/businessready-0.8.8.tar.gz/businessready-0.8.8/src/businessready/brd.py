...
def ISE_deployment_info(url, username, password):
    try:
        ...
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = deployment_template.render(deploymentInfoJSON = deploymentInfoJSON['ERSDeploymentInfo'],filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
    ...
    # -------------------------
                ...
                with open(f"ISE Deployment Info.json", "w") as fh:
                    json.dump(deploymentInfoJSON, fh, indent=4, sort_keys=True)
                    fh.close()                            
