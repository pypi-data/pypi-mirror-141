...
def extension_request(request, retrieve_by, properties = None, out_log = None, global_log=None):
    ...
    for k in properties.keys():
        ...
        if any(str(sub) in k for sub in request.split('/')): 
            continue
        elif any(str(sub) in properties[k] for sub in request.split('/')):
            continue
        else:
...
def convert_file(output_file_tc, output_file_c, format_file, out_log=None, global_log=None):
    ...
    df.to_csv(output_file_c+format_file, columns = header,index=False, sep='\t', mode="a")
