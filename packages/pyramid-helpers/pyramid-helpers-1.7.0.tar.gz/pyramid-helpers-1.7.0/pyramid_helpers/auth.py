...
def includeme(config):
    ...
    if settings['auth']['mode'] not in AUTH_CLIENTS:
        raise ConfigurationError(f'[AUTH] Invalid value for parameter mode in section [auth]: {settings["auth"]["mode"]}')

    # Check hashalg
    if settings['policy:ticket']['hashalg'] not in hashlib.algorithms_available:
        raise ConfigurationError(f'[AUTH] Invalid value for parameter hashalg in section [policy:ticket]: {settings["auth"]["policy:ticket"]["hashalg"]}')
