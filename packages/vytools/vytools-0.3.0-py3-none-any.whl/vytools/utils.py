...
def get_repo_from_path(path, repo_path_list):
  if repo_path_list is None:
    logging.error('"info:repository_path_list" does not exist in the list of items')
  else:
...
def ok_dependency_loading(action,type_name,items):
  if type_name not in items or not items[type_name]['loaded']:
    logging.error('Cannot {} "{}" because it did not load properly'.format(action,type_name))
    return False
...
def _check_self_dependency(id,item):
  ...
  if n != len(item['depends_on']):
    item['loaded'] &= False
    logging.error('Item "{n}" should not depend on itself'.format(n=id))
