...
def dependencies(stage_text, items):  
  dependency = []
  build_context = '.'
  external_images = []
  success = True
...
def parse(namein, pth, items):
  ...
  (success, dependency, args, build_context) = dependencies(stage_text, items)
  if not success:
    logging.error('Stage "{}" references a build context "{}" which does not exist'.format(name,build_context))
    return False
  elif os.path.isabs(build_context):
    logging.error('Stage "{}" has build context "{}" which is an absolute path. It should be relative to the .stage.dockerfile'.format(name,build_context))
    return False
  else:
    try:
      if build_context.startswith('vydir:'):
        if build_context in items:
          ...
          logging.error('Stage build context "{p}" for stage {n} is not a known vydir'.format(p=build_context,n=name))
          return False
      elif build_context.startswith('repo:'):
        if build_context in items:
          build_context = Path(items[build_context]['path']).resolve()
        else:
          logging.error('Stage build context "{p}" for stage {n} is not a known repo'.format(p=build_context,n=name))
          return False
      else:
