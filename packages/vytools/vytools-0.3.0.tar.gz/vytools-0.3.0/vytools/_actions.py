...
def upload(original_list, url, uname, token, check_first=True, items=None):
  if items is None: items=ITEMS
  results = {}
  if len(original_list) == 0:
    results = vytools.bundle.upload_all(url, uname, token, check_first, items=items)
  else:
    for type_name in original_list:
      if type_name.startswith('bundle:'):
        results[type_name] = vytools.bundle.upload(type_name, url, uname, token, check_first, items=items)
  return results
