...
class OptionData:
    ...
    def __init__(self, name, description, required=False, choices=None, options=None, **kwargs):
        ...
        if self.type is None:
            raise errors.IncorrectCommandData("type is required for options")
        ...
        if self.type in (1, 2):
            ...
            if options is not None:
                ...
                raise errors.IncorrectCommandData(
...
class ComponentMessage(discord.Message):
    ...
    def get_component(self, custom_id: int) -> dict:
        """
...
class CallbackObject:
    ...
    def _prepare_cooldowns(self, ctx):
        ...
        if self._buckets.valid:
            ...
            if retry_after:
                raise discord.ext.commands.CommandOnCooldown(bucket, retry_after)
...
class ComponentCallbackObject(CallbackObject):
    ...
    def __init__(
        ...
        if component_type not in (2, 3, None):
            raise errors.IncorrectFormat(f"Invalid component type `{component_type}`")
...
class SlashMessage(ComponentMessage):
    ...
    def __init__(self, *, state, channel, data, _http:discord.http, interaction_token):
