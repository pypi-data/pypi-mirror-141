    ...
    def get_dic(self, *args, _model="LD", **kwargs) -> list or dict:
        ...
        if isinstance(_model, str):
            if _model == "LD":
                if args:
                    return list(
                        map(lambda x: {arg: x.get(arg, None) for arg in args}, self.data_list)
                    )
                else:
                ...
                if args:
                    return {
                        attr: list(map(lambda x: x.get(attr, None), self.data_list)) for attr in args
                    }
                else:
                    return {
                        attr: list(map(lambda x: x.get(attr, None), self.data_list)) for attr in self.attribute
                    }
