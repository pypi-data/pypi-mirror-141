...
class EasySql:
    ...
    def find(self, *args, table_name="", **kwargs):
        ...
        if kwargs:
            ...
            return_data = DataSql(
                self.database, table_name, args or self.cursor, self.get_data(
                    _find_language), find_info=kwargs)
        else:
            _find_language = """SELECT %s FROM `%s`""" % (select_str, table_name)
            return_data = DataSql(
                self.database, table_name, args or self.cursor, self.get_data(_find_language), find_info=None)
    ...
    def find_by_sql_str(self, *args, table_name="", sql_str=""):
        ...
        if sql_str:
            return_data = DataSql(
                ...
                table_name,
                args or self.cursor,
                self.get_data("""SELECT %s FROM `%s` WHERE %s""" % (select_str, table_name, sql_str)),
                find_info=sql_str
            )
        else:
            return_data = DataSql(
                ...
                table_name,
                args or self.cursor,
                self.get_data("""SELECT %s FROM `%s`""" % (select_str, table_name)),
                find_info=None
            )
        return_data.data_list = True
        return return_data
    ...
    def update_values(self, *args, table_name="", find_info=None, update_datas=None):
        if args and table_name and find_info and update_datas and isinstance(update_datas, (tuple, list)):
            if isinstance(find_info, str):
                find_info_used = _find_info_read(find_info).keys()
            elif isinstance(find_info, dict):
                find_info_used = find_info.keys()
            elif isinstance(find_info, (tuple, list)):
