...
def get_trashtypes_by_trashcat_repr(general_tables_dict, trash_types_table_name,
    ...
    trash_cats_map = get_table_info(general_tables_dict, 'trash_cats_types')
    trash_types_list = get_trashtypes_by_trashcat_id(trash_types_table_info, trash_cat_id, trash_cats_map)
    trash_types_rers = [get_repr_by_id_table(general_tables_dict, 'trash_types',x) for x in trash_types_list]
    return trash_types_rers


def get_trashtypes_by_trashcat_id(trash_types_dict, trash_cat_id, trash_cats_map, trash_cat_column_name='category', *args, **kwargs):
    ...
    for ident, dictname in trash_cats_map.items():
        if dictname['trash_cat_id'] == trash_cat_id:
            matched.append(dictname['trash_type_id'])
    return matched
