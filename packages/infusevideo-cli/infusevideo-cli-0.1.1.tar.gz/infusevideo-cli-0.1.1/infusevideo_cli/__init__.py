...
First use:
  --generate-config=ORG   generates a configuration file with ORG set as
                          organization for the default profile.
...
class CommandlineOptions(BaseSettings):
	generateConfig: Optional[str] = Field(alias="--generate-config")
...
def cli() -> None:
	...
	if options.generateConfig is not None:
		try:
			path = infusevideo.Config.generate_sample_config(options.generateConfig)
	...
	except infusevideo.errors.ConfigNotFound:
		print("No configuration file found. Create a default one by running:", file=sys.stderr)
		print(f"  {sys.argv[0]} --generate-config ORGANIZATION", file=sys.stderr)
		print("replacing ORGANIZATION with your organization ID (org_...).", file=sys.stderr)
		sys.exit(1)
