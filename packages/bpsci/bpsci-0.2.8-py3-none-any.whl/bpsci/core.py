...
class anim_text:
    def __init__(self, name, parent, anim):
        ...
        bpy.data.curves.new(type="FONT", name="Font Curve").body = "INIT"
        font_obj = bpy.data.objects.new(name=name+'_text', object_data=bpy.data.curves["Font Curve"])
        bpy.context.scene.collection.objects.link(font_obj)

        self.obj = bpy.data.objects[name+'_text']

        self.obj.parent = parent
        self.name = name


    def animate(self, data, label, fix_place, if_str):
        anim = self.anim

        if not(if_str):
            def update(self):
                text = bpy.data.objects[self.name+'_text']
