...
def test_loc_set_columns(loc_sets):
    ...
    for col_name in expected_sub_cols:
        if LocSetSharedCols.must_exist(col_name=col_name):
            assert col_name in loc_sets.subloc.geo_df_original.columns
            assert LocSetSharedCols.get_simple_col_name(col_name=col_name) in loc_sets.subloc.geo_df.columns
