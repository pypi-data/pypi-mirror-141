...
class LocationSetBase:
    ...
    def geo_df(self) -> gpd.GeoDataFrame:
        ...
        # use simple columns (e.g. rename 'LOC_ID' to 'InternalLocation'. The latter is used in id-mapping)
        self._geo_df.rename(
            columns={orig: LocSetSharedCols.get_simple_col_name(col_name=orig) for orig in self._geo_df.columns},
            inplace=True
        )
        return self._geo_df
