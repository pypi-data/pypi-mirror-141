...
class _Context:
    client: dw_client.DeskWatchClient


@click.group(
    help="CLI utility for dw-client to aid in interacting with the DeskWatch server"
)
@click.option(
    "--host",
    default="127.0.0.1",
...
def main(ctx, testing: bool, host: str, port: int):
    ctx.obj = _Context()
    ctx.obj.client = dw_client.DeskWatchClient(
        host=host,
