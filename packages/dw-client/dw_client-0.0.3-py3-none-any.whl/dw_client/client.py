...
from dw_core.models import Event
from dw_core.dirs import get_data_dir
...
class DeskWatchClient:
    def __init__(
        ...
        """
        A handy wrapper around the dw-server REST API. The recommended way of interacting with the server.
    ...
    def heartbeat(
        ...
        Args:
            ...
            pulsetime: The maximum amount of time in seconds since the last heartbeat to be merged with the previous heartbeat in dw-server
            queued: Use the dw-client queue feature to queue events if client loses connection with the server
        ...
        from dw_transform.heartbeats import heartbeat_merge
...
class RequestQueue(threading.Thread):
    ...
    def __init__(self, client: DeskWatchClient) -> None:
        ...
        data_dir = get_data_dir("dw-client")
    ...
    def _try_connect(self) -> bool:
        try:  # Try to connect
            ...
            self.connected = True
            logger.info(
                "Connection to dw-server established by {}".format(
                    self.client.client_name
                )
            )
    ...
    def _dispatch_request(self) -> None:
        ...
        try:
            ...
            self.connected = False
            logger.warning(
                "Failed to send request to dw-server, will queue requests until connection is available."
            )
            return
