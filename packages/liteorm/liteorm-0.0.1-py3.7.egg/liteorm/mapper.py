...
from liteorm.info import DataBaseInfo
...
<xml>
    ...
    {% endfor %}
            1 = 1
            {{ "{"|echo }}% if (sqlite_mapper_order_by | default("")) != "" %{{ "}"|echo }} 
                ORDER BY {{ "{"|echo }}{ sqlite_mapper_order_by }{{ "}"|echo }} 
            {{ "{"|echo }}% endif %{{ "}"|echo }} 
            {{ "{"|echo }}% if (sqlite_mapper_limit_start | default(-1)) != -1 %{{ "}"|echo }} 
                LIMIT #{ sqlite_mapper_limit_start }, #{ sqlite_mapper_limit_length | default(10) } 
            {{ "{"|echo }}% endif %{{ "}"|echo }}
        </value>
    </sql>
    <sql>
...
def get_mapper_xml(database_info: DataBaseInfo, table_name: str) -> str:
    ...
    data["int_types"] = ["INTEGER"]
    data["float_types"] = ["REAL"]
    data["date_types"] = []
    data["string_types"] = ["TEXT"]
