...
class ElasticAggregator:
    ...
    def facts(self, size=30, filter_by_fact_name=None, min_count=0, max_count=None, include_values=True, include_doc_path=False):
        """
                        ...
                        if fact_value["key"] and fact_value["doc_count"] > min_count:
                            if max_count:
                                if fact_value["doc_count"] < max_count:
                                    if include_doc_path:
                                        entities[fact_name].append({"value": fact_value["key"],
                                                                    "doc_path": fact_value["doc_paths"]["buckets"][0][
                                                                        "key"]})
                                    else:
