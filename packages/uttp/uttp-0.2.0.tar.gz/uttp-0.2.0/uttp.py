...
class webserver:
    ...
    def _find_url_handler(self, req):
        ...
        # Third try - see if it matches any regex urls
        for pattern, f, params in self.regex_urls:
            match = pattern.match(req.path)
            if match:
              print('match', match)
              num_groups = int(repr(match).split('=')[-1].rstrip('>'))
              req._params = [match.group(i).decode() for i in range(1, num_groups)]
              return f, params
    ...
    async def _handler(self, reader, writer):
        ...
        try:
            ...
            elif hasattr(req, '_params'):
                ret = await req.handler(*[req, resp]+req._params)
            else:
    ...
    def add_route(self, url, f, **kwargs):
        ...
        if url.__class__.__name__=='ure':
          # url is a regex
            self.regex_urls.append((url, f, params))
        else:
          if url == '' or '?' in url:
