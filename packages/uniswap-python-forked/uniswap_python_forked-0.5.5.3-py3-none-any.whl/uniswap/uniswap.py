...
class Uniswap:
    ...
    def make_trade(
        ...
        if route is None:
            if self.version == 2:
                ...
                # BL: Default route should be token0 -> token1, for Blueblock's clients
                route = [input_token, output_token]
                logger.warning(f"BB: Route is undefined, assuming route: {route}")
    ...
    def _token_to_token_swap_input(
        ...
        if self.version == 1:
            ...
            logger.debug("#DEBUG qty, min_tokens_bought, input_token, self.get_weth_address(), output_token, recipient", 
                qty, min_tokens_bought, input_token, self.get_weth_address(), output_token, recipient)
            return self._build_and_send_tx(
                func(
                    qty,
                    min_tokens_bought,
                    route, # BL: pass in route from params.
                    recipient,
                    self._deadline(),
                ),
            )
