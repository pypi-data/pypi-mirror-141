...
######################################
from .pynestor import NestorDescSet, ScriptNestorInstance, NestorGitDesc, Utils, NestorOpt
...
class EnvironementConfig():
    ...
    def __init__(self, base_env_vars=None):
        ...
        self.NESTOR_NAME = base_env_vars.get('NESTOR_NAME') or base_env_vars.get('CI_COMMIT_REF_NAME_SLUG')
...
class InteractiveConfig(EnvironementConfig):
    def __init__(self):
        ...
        log(f"Nom du projet git (Sans le 'odoo-addons/' ex: mettre `aef` pour `odoo-addons/aef`) ?")
...
class PreviewUtils:
    ...
    def _create_all_spec_sources(self, main_project_path):
        ...
        spec_src = self.create_spec_sources(idx=0, project_path=main_project_path, branch_to_test=[
        ...
        for depend in self.env_config.ODOO_DEPENDS.split(","):
            if depend:
                nestor_values.add(
                    self.create_spec_sources(idx=idx, project_path=depend, branch_to_test=[
                        self.env_config.CI_COMMIT_REF_NAME,
                        current_gl_project.default_branch,
                        self.env_config.ODOO_VERSION
                    ]))
...
class ScriptNestor():
    ...
    def enable_s3_spec(self, values):
        if values['persistence.s3.secret']:
            values.add(self.inst.filestore.enable_on_s3_spec(s3_secret="s3-ndp-preview", s3_bucket=self.config.S3_BUCKET, values=values))
        return values
...
class PreviewUpScript(ScriptNestor):
    ...
    def run_script(self):
        ...
        if not self.inst.exist():
            log("Create instance")
            values = self.preview_utils.get_spec_values(sections=[
                "default",
                "preview",
                self.config.CI_COMMIT_REF_NAME
            ], stage="restore")
