...
intro_text = '欢迎使用TopFintech数据API。所有数据由人工智能程序收集、整理、计算，仅供参考。新手请查阅用户手册：http://www.topfintech.org/TF_API。屏蔽本登录提示: self.__init__(slient=True)'
...
get_daily_text = 'get_daily\n简介: 提供股票、ETF、指数，OHLC,成交额、成交量日线数据\n参数：code - 证券代码,必填，详阅self.menu("code"); start_date/end_date/sample_size - 交易日序列参数，必填，详见self.menu("date_range"); timeout - 等待服务器返回数据的最长时秒, 默认200秒\n返回：时间序列表格日线数据'
