...
class Process(EventRegistryAware):
    ...
    def to_json(self):
        import json
        return json.dumps(self.to_json2(self))

    def to_json2(self, object) -> dict:
        obj = {}
        ...
        for k in (object.__dict__ if not isinstance(object, dict) else object):
            ...
            if type(value) == list:
                obj[k] = [{"type": type(x).__name__, "item": self.to_json2(x)} for x in value]
            elif type(value) == dict:
                obj[k] = {"type": type(value).__name__}
                result = self.to_json2(value)
