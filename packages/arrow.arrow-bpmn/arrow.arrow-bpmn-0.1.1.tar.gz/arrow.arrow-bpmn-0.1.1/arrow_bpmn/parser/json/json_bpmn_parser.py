...
class JsonBpmnParser(BpmnParser):
    ...
    def _parse_task(self, element: JSONElement):
        ...
        if _type == "BusinessRuleTask":
            ...
            return BusinessRuleTask(_item.as_dict(), implementation)
        if _type == "CallActivity":
            ...
            return CallActivity(_item.as_dict(), incoming_state, outgoing_state)
        if _type == "ManualTask":
            return ManualTask(_item.as_dict())
        if _type == "UserTask":
            return UserTask(_item.as_dict())
        if _type == "ReceiveTask":
            return ReceiveTask(_item.as_dict())
        if _type == "ScriptTask":
            return ScriptTask.from_json(element)
        if _type == "SendTask":
            return SendTask(_item.as_dict())
        if _type == "ServiceTask":
            return ServiceTask(_item.as_dict())
