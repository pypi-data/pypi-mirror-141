...
class SequentialBpmnEngine(BpmnEngine):
    ...
    def invoke_by_event(self, event: Event, init_state: Optional[dict] = None) -> List[State]:
        def invoke(subscription: NodeRef):
            ...
            environment = Environment(event.group, process)
    ...
    def _handle_action(self, action: Action, state: State, env: Environment) -> List[State]:
        ...
        if isinstance(action, ContinueAction):
            node = env.get_node(action.id)
            assert node is not None, f"no node found with id {action.id}"
            new_state = state.with_reference(NodeRef(state.reference.process_id, action.id))
        ...
        # **************
            ...
            new_environment = Environment(event.group, process)
        ...
        # *************
            ...
            if action.reference.process_id != env.process_id:
                ...
                _env = Environment(env.group, parent_process)
