...
from arraytool.operator import OperatorTransfer
...
class ArraySeries:
    ...
    def setdiff(self, other):
        ...
        if isinstance(other,ArraySeries):
            return self.new_data(np.setdiff1d(self._ValidData,other._ValidData))
        elif isinstance(other,Series):
            return self.new_data(np.setdiff1d(self._ValidData,other.values))
        else:
            return self.new_data(np.setdiff1d(self._ValidData,other))
...
class ArrayFrame:
    ...
    def _SetLoc(self, index = None, columns = None, 
        ...
        if index is None and columns is None \
            ...
            self.loc = ArrayFrameLocIndexer(
                self,index,columns,index_cols,col_index,copy)
            self._NoLoc = False
        self.index = self.loc.index
    ...
    # 2022-03-06
    def new_columns(self, new_frame = True):
        '''创建新列索引'''
        return new_index(self.columns,new_frame)
    ...
    # 2022-03-06
    def new_index(self, new_frame = True):
        '''创建新行索引'''
        return new_index(self.index,new_frame)
    ...
    def set_columns(self, columns = None, col_index = None, copy = False, 
        ...
        if self._NoLoc:
            ...
            self.loc.set_index(columns,col_index,1,copy,_assign_manager)
            self.columns = self.loc.columns
    ...
    def set_index(self, index = None, index_cols = None, copy = False, 
        ...
        if self._NoLoc:
            ...
            self.loc.set_index(index,index_cols,0,copy,_assign_manager)
            self.index = self.loc.index
        return
...
class ArrayFrameLocIndexer:
    ...
    def __init__(self, frame, index = None, columns = None, 
        index_cols = None, col_index = None, copy = False, iloc = False):
    ...
    # 2021-07-19
    def set_index(self, index = None, index_cols = None, axis = 0, 
        copy = False, _assign_manager = None):
        '''
        设置索引。
        【注】
        1. axis为0对应行/index，为1对应列/columns。
        2. index优先级高于index_cols。
        '''
        frame = self.frame
        if self._ILoc or index is None and index_cols is None \
            or type(index) is ArrayIndex or IsSimpleIndex(index):
            if axis==0:
