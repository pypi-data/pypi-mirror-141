class Constants(object):
    ...
    window.executePython = function(python, kernelId) {
        if (typeof Jupyter !== 'undefined') {
            return new Promise((resolve, reject) => {
             ...
             return new Promise((resolve, reject) => {
                if (JupyterLab && JupyterLab[kernelId]) {
    ...
    function cfMessageEventListener(event) {
        ...
        if (python_code !== "") {
            ...
            window.executePython(python_code, kernelId).then(result => window.sendMessage(result, `${event.data.iframe}`, `${event.data.action}`, `${event.data.reqId}`));
        }        
    }
    """
