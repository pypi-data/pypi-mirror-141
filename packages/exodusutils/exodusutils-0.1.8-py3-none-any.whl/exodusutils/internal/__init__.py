...
def validate_columns(header: List[str], features: List[Column]) -> None:
    ...
    for f in features:
        if f.name not in header: # This means that a required field is missing from the dataframe
            raise ExodusBadRequest(f"column {f.name} is missing from the prediction data")
...
def get_column_mode(col) -> float:
    ...
    """
    mode = col.mode().values[0] # Just take the first one if there are ties
    return 0.0 if math.isnan(mode) else mode
