class BodyHelper:
    ...
    def __append_query(self, clause, query):
        if clause == 'must':
            self.bool_query['bool']['must'].append(query)
        elif clause == 'should':
            self.bool_query['bool']['should'].append(query)
        elif clause == 'must_not':
            self.bool_query['bool']['must_not'].append(query)
...
if __name__ == '__main__':
    helper = BodyHelper()
    helper.match_phrase('blog.raw_text','战争','should').term('blog.id','12345')
