...
class DeviceEposChannel(DeviceChannel):
    ...
    def get_device_firmware_version(self):
        ...
        self.device.check_error(
            self.device.lib.VCS_GetVersion(
                self.device.handle,
                self.channel,
                byref(hw_version),
                byref(sw_version),
    ...
    def get_movement_state(self):
        state = c_int()
        self.device.check_error(
            self.device.lib.VCS_GetMovementState(
                self.device.handle,
                self.channel,
                byref(state),
                byref(self.device.pError),
    ...
    def get_enabled_state(self):
        state = c_int8()
        self.device.check_error(
            self.device.lib.VCS_GetEnableState(
                self.device.handle,
                self.channel,
                byref(state),
                byref(self.device.pError),
    ...
    def get_operation_mode(self):
        ...
        self.device.check_error(
            self.device.lib.VCS_GetOperationMode(
                self.device.handle,
                self.channel,
    ...
    def get_velocity_units(self):
        ...
        notation = c_int8()
        self.device.check_error(
            self.device.lib.VCS_GetVelocityUnits(
                self.device.handle,
                self.channel,
                byref(dimension),
                byref(notation),
    ...
    def get_velocity(self):
        velocity = c_int64()
        self.device.check_error(
            self.device.lib.VCS_GetVelocityIsAveraged(
                self.device.handle,
                self.channel,
                byref(velocity),
                byref(self.device.pError),
    ...
    def get_velocity_profile(self):
        ...
        deceleration = c_int()
        self.device.check_error(
            self.device.lib.VCS_GetVelocityProfile(
                self.device.handle,
                self.channel,
                byref(acceleration),
                byref(deceleration),
    ...
    def get_position(self):
        position = c_int64()
        self.device.check_error(
            self.device.lib.VCS_GetPositionIs(
                self.device.handle,
                self.channel,
                byref(position),
                byref(self.device.pError),
        ...
        self.set_raw_value(
            ATTR_REVOLUTIONS, int(attribute.apply_offset_factor(position.value) / 360)
        )
    ...
    def get_position_profile(self):
        ...
        deceleration = c_int()
        self.device.check_error(
            self.device.lib.VCS_GetPositionProfile(
                self.device.handle,
                self.channel,
                byref(velocity),
                byref(acceleration),
    ...
    def set_operation_mode(self, value):
        ...
        self.device.check_error(
            self.device.lib.VCS_SetOperationMode(
                self.device.handle,
                self.channel,
    ...
    def set_velocity(self, velocity):
        ...
        self.device.check_error(
            self.device.lib.VCS_MoveWithVelocity(
                self.device.handle,
                self.channel,
                c_int64(velocity),
                byref(self.device.pError),
    ...
    def set_absolute_position(self, position, absolute=True):
        ...
        self.device.check_error(
            self.device.lib.VCS_MoveToPosition(
                self.device.handle,
                self.channel,
                c_int64(position),
                c_int(absolute),
    ...
    def set_motor_enabled(self, value):
        ...
        if value:
            ...
            self.device.check_error(
                self.device.lib.VCS_SetEnableState(
            ...
            self.device.check_error(
                self.device.lib.VCS_SetDisableState(
                    self.device.handle, self.channel, byref(self.device.pError)
                )
            )
    ...
    def set_velocity_units(self, value):
        ...
        self.device.check_error(
            self.device.lib.VCS_SetVelocityUnits(
                self.device.handle,
                self.channel,
                dimension,
                notation,
    ...
    def set_velocity_profile(self, acceleration=None, deceleration=None):
        ...
        self.device.check_error(
            self.device.lib.VCS_SetVelocityProfile(
                self.device.handle,
                self.channel,
                acceleration,
                deceleration,
    ...
    def set_position_profile(self, velocity=None, acceleration=None, deceleration=None):
        ...
        self.device.check_error(
            self.device.lib.VCS_SetPositionProfile(
                self.device.handle,
                self.channel,
                velocity,
                acceleration,
    ...
    def stop(self):
        self.device.check_error(
            self.device.lib.VCS_HaltVelocityMovement(
                self.device.handle, self.channel, byref(self.device.pError)
            )
        )
        self.set_motor_enabled(False)
