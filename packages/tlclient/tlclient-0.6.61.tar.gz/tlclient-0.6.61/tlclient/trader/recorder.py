...
class MongoClient(RecorderClient):
    ...
    def __init__(self, logger, config_path, collection, db_name):
        RecorderClient.__init__(self, logger)
        self._settings = Configurator(config_path).get_mongo_settings()
        ...
        try:
            print(mongo_client.server_info(), db_name)
        except Exception:
            print("Unable to connect to the server.")
        if db_name is None:
            db_name = self._settings.db
        self._client = mongo_client[db_name]
    ...
    def _proc_write_thread(self, res):
        try:
            ...
            self._client[self._collection].insert_many(res)
            writes_time = time.time() - start
            if writes_time < 1:
    ...
    def writes(self, msg_obj, f):
        if hasattr(msg_obj, 'to_dict'):
            ...
            mongo_data = MessageToDict(msg_obj)
        else:
...
class Recorder(Client):
    ...
    def __init__(self, name, env_name, addr):
        super().__init__(name, env_name, addr)
        self.clients = []
        for one in [
    ...
    #     self.logger.info(f'[on_pub_frame] (f){f}')
    #     # TODO: need this returned value
    #     obj = super().on_pub_frame(f)
    #     if obj is not None:
    #         self.process_msg_obj(obj, f)
