...
class MCViewDashboard(BaseDashboard):
    ...
    def get_move_obs(self) -> MoveObserver:
        obs = self.app_data.get("_mc_step_obs", None)
        if obs is None:
            raise ValueError(
    ...
    def _play_mc(self):
        ...
        for ii, step in enumerate(move_obs.steps, start=1):
            if self._event.is_set():
                ...
                break
            atoms = _build_next_atoms(atoms, step)
...
def _build_next_atoms(atoms: ase.Atoms, step: MCStep) -> ase.Atoms:
    """Helper function to apply a (canonical) MC step to an atoms object.
    Assumes the step corresponds to a swap."""
    if not step.move_accepted:
        return atoms
    swap = [change.index for change in step.last_move]
    if not len(swap) == 2:
        raise RuntimeError("Change must be a swap")
    atoms = atoms.copy()
    idx_rev = swap[::-1]
    atoms.positions[idx_rev] = atoms.positions[swap]
    return atoms
