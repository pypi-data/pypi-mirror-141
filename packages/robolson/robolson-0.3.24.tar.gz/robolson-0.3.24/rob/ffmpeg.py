...
def spin_up(folder):
    ...
    rich.print(f"[green]Working on job #{_JOBS_DONE+1}...[/green]\n")
    _JOBS_DONE += 1
...
def interact():
    ...
    choice = input(f"Enter path to work on [default: {pathlib.Path(_PATH).absolute()}].\n{_PROMPT}")
    if choice:
        _PATH = choice

    rich.print(f"[yellow]Working on '{pathlib.Path(_PATH).absolute()}'.\n")
