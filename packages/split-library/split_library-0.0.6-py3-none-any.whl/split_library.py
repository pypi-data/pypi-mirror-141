...
def unlimited_homogeneous_veh_split(graph_distance_matrix):
    ...
    p[0] = 0
    pred: ndarray = np.asarray([-1 for i in range(n_location)], dtype='int64')
...
def heterogeneous_split(all_veh, graph_distance_matrices):
    ...
    def helper_heterogeneous_split(key_list, graph_distance_matrices):
        n_location = len(graph_distance_matrices[0])
        ...
        return pred, p, best_veh, veh_order

    return helper_heterogeneous_split(key_list, graph_distance_matrices)
