...
def initialize_training(layer):
    ...
    Returns:
        [`AdaRoundQuantizer`], [`tf.Variable`]: a list of AdaRound quantizers and a list of trainable variables
...
def finalize_training(layer):
    ...
    if isinstance(layer, QuantizedSeparableConv2D):
        ...
        w_list = layer.get_weights()
        print_console("Changed dw weights: {} / {}".format(
            np.sum(
                layer.quantizer_dw.quantize(w_list[0]) !=
                layer.quantizer_dw.quantize(adaround_dw)),
