...
def log(socket: socket.socket, data: bytes, header: bytes = b"") -> None:
    pid = os.getpid()
    with open(f"/tmp/memcache.socket-{pid}-{socket.fileno()}.log", "ab") as fp:
        # greenlet_id = "<Unknown>"
        # try:
        ...
        fp.write(header)
        fp.write(data)
        # fp.write(f"     [ Thread {thread_id} / Greenlet {greenlet_id}]\n".encode())
        fp.write(f"     [ Thread {thread_id} ]\n".encode())
        fp.flush()
...
class MemcacheSocket:
    ...
    def __init__(
        ...
        log(self._conn, f"*** NEW CONN {self}".encode())

    def __str__(self) -> str:
        return f"<MemcacheSocket {id(self)} - {self._conn.fileno()}>"
    ...
    def get_response(
        ...
        try:
            ...
            log(self._conn, f"*** Error reading header from socket {e}".encode())
            _log.exception(f"Error reading header from socket {self._conn}")
            raise MemcacheError(f"Error reading from socket: {e}") from e
        ...
        try:
            ...
            _log.exception(f"Error in response in socket {self._conn}")
            raise MemcacheError(
    ...
    def get_value(self, size: int) -> Union[bytearray, memoryview]:
        ...
        log(self._conn, f"<DATA {size}>".encode(), b"< ")
        return data
