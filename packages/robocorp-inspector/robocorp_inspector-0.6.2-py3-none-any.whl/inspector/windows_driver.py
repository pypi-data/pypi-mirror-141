...
from RPA.core.windows import Window, WindowsElements, ElementInspector

from .windows.base import APP_WINDOW_TITLE
...
WindowsElement = Dict[str, Union[str, auto.Control]]
...
class WindowsDriver:
    def __init__(self):
        ...
        self._windows_elements = WindowsElements(None)
        self._window = Window()
        self._active_window = ""
        self.queue = Queue()
    ...
    def _on_click(
        ...
        try:
            ...
            ElementInspector.inspect_element(None, True, elements)
    ...
    def list_windows(self) -> OpenWindows:
        ...
        windows = []
        try:
            windows = self._window.list_windows(icons=True)
        ...
        for window in windows:
            if window["title"] and window["title"] != APP_WINDOW_TITLE:
                ...
                if window["icon"]:
                    win["icon"] = window["icon"].decode("utf-8")
