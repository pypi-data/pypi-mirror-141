...
def colprint(d,header=None,key=None,indent="",crop=True,format='tree',fields=None,fieldcolors=None,enum=False):
	if format=="tree":
		colprint_tree(d,header,key,indent,crop)
		return
	if format=="column":
		colprint_column(d,header,fields=fields,fieldcolors=fieldcolors,enum=enum)
		return

def colprint_column(data,header=None,fields=None,fieldcolors=None,short=False,enum=False):
	columncolors=[
	"yellow",
	"lightblue",
	"pink",
