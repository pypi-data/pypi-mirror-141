"""
To increase the speed in the subproblems, this modifies the projected nlp to 
take an argument with primals_names in order to avoid doing it twice for
each model.
"""
...
class ProjectedNLP(_BaseNLPDelegator):
    def __init__(self, original_nlp, primals_ordering, original_primals_names=None):
        """
        ...
        self._original_primals_names = original_primals_names
        self._generate_maps()
    ...
    def _generate_maps(self):
        if self._original_idxs is None or self._projected_idxs is None:
            ...
            if self._original_primals_names is None:
                original_names = self._original_nlp.primals_names()
            else:
                original_names = self._original_primals_names
            original_idxs = list()
            projected_idxs = list()
