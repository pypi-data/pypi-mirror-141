...
def test_thickness_estimates__diagonal_layers():
    ...
    voxel_dimensions = np.array([10. , 10., 10.], dtype=np.float32) * np.sqrt(2)
    ...
    thicknesses = estimate_thicknesses(
        layers,
        vector_field,
        offset=np.array([5., 5., 5.], dtype=np.float32),
    ...
    # voxel in layer 1
    npt.assert_allclose(
        slice_thicknesses[19, 26],
        [350.,  80.,  60.,  80.,  60., 80.,  60.],
        atol=0.1
    )

    # voxel in layer 2
    npt.assert_allclose(
        slice_thicknesses[18, 25],
        [330.,  80.,  60.,  80.,  60., 80.,  60.],
        atol=0.1
    )

    # voxel in layer 3
    npt.assert_allclose(
        slice_thicknesses[16, 18],
        [250.,  60.,  80.,  60.,  80., 60.,  80.],
        atol=0.1
    )

    # voxel in layer 4
    npt.assert_allclose(
        slice_thicknesses[9, 15],
        [150.,  60.,  80.,  60.,  80., 60.,  80.],
        atol=0.1
    )

    # voxel in layer 5
    npt.assert_allclose(
        slice_thicknesses[11, 11],
        [130.,  60.,  80.,  60.,  80., 60.,  80.],
        atol=0.1
    )

    # voxel in layer 6
    npt.assert_allclose(
        slice_thicknesses[3, 7],
        [10.,  60.,  80.,  60.,  80., 60.,  80.],
        atol=0.1
    )
