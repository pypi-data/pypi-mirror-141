...
class PlotBase:
    ...
    def generate_figure(self, df: DataFrame, query=None, x=None, y=None, z=None, group_by=None, groups=None,
        ...
        if group_by:
            if groups:
                query = " or ".join([f"(`{group_by}` == {as_string(x)})" for x in groups])
                ...
                _sub_chart = chart_data.query(f"`{group_by}` == {as_string(x_grp)}")[idx_col].to_frame()

                if x is not None:
                    _df = chart_data.query(f"`{group_by}` == {as_string(x_grp)}")[[idx_col,x]].rename(columns={x: _x})
                    _sub_chart = _sub_chart.merge(_df, how='outer', left_on=idx_col, right_on=idx_col)

                if y is not None:
                    _df = chart_data.query(f"`{group_by}` == {as_string(y_grp)}")[[idx_col,y]].rename(columns={y: _y})
                    _sub_chart = _sub_chart.merge(_df, how='outer', left_on=idx_col, right_on=idx_col)

                if z is not None:
                    _df = chart_data.query(f"`{group_by}` == {as_string(z_grp)}")[[idx_col,z]].rename(columns={z: _z})
                    _sub_chart = _sub_chart.merge(_df, how='outer', left_on=idx_col, right_on=idx_col)
        ...
        if group_by is not None and groups:
            ...
            annotations = []
            for grp in groups:
                sub = chart_data.query(f"`{group_by}` == {as_string(str(grp).strip())}")
