...
class ExpectationValue(Node):
    ...
    See Also
    --------
    inner_product : Inner prodcut of two vectors.
    ...
    args = [
        forge.arg("state", type=Union[np.ndarray, Tensor]),
    ...
    def create_node_data(cls, _operation, **kwargs):
        ...
        check_argument(
            isinstance(state, (Tensor, np.ndarray)),
            "State must be a Tensor or a np.ndarray.",
            {"state": state},
        )
        check_argument(
            isinstance(operator, (Tensor, np.ndarray)),
...
class InnerProduct(Node):
    ...
    For more information about the inner product, see `dot product on Wikipedia`_.

    .. _dot product on Wikipedia:
...
class OuterProduct(Node):
    ...
    For more information about the outer product, see `outer product on Wikipedia`_.

    .. _outer product on Wikipedia:
...
class UnitaryInfidelity(Node):
    ...
    Parameters
    ----------
    unitary_operator : Tensor or np.ndarray
    ...
    args = [
        forge.arg("unitary_operator", type=Union[np.ndarray, Tensor]),
        forge.arg("target", type=Union[np.ndarray, Tensor]),
    ]
    rtype = Tensor
    categories = [Category.MANIPULATING_TENSORS]
