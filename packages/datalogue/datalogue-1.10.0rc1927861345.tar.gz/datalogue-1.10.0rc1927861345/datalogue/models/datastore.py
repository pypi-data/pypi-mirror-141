...
class DatastoreDef(ABC):
    ...
    def __init__(
        self, credential_type: CredentialType, datastore_id: Optional[UUID] = None
    ):
...
def _s3_datastore_def_from_payload(d: dict) -> Union[DtlError, S3DatastoreDef]:
    ...
    if not isinstance(type_field, str):
        return DtlError(
            "string %s is missing from the json" % (DatastoreDef.type_field)
        )
...
def _avc_datastore_def_from_payload(d: dict) -> Union[DtlError, AVCOrderItemsDatastore]:
    ...
    if type_field != AVCOrderItemsDatastore.type_str.value:
        return DtlError(
            "The object %s is not a AVCOrderItemsDatastore definition" % (str(d))
        )

    marketplace_id = d.get("marketplaceId")
    if marketplace_id is None:
        return DtlError(
            "'marketplaceId' needs to be defined in a AVCOrderItemsDatastore definition"
        )

    created_after = map_option(d.get("createdAfter"), _parse_datetime)
    if created_after is None:
        return DtlError(
            "'createdAfter' needs to be defined in a AVCOrderItemsDatastore definition"
        )

    created_before = map_option(d.get("createdBefore"), _parse_datetime)
    store_id = d.get(DatastoreDef.id_field)

    return AVCOrderItemsDatastore(
        marketplace_id, created_after, created_before, store_id
    )
...
def _azure_datastore_def_from_payload(d: dict) -> Union[DtlError, AzureDatastoreDef]:
    ...
    if container is None:
        return DtlError(
            "'container' needs to be defined in an Azure Datastore definition"
        )

    file_name = d.get("fileName")
    if file_name is None:
        return DtlError(
            "'fileName' needs to be defined in an Azure Datastore definition"
        )

    file_format = d.get("format")
    if file_format is None:
        return DtlError(
            "'file_format' needs to be defined in an Azure Datastore definition"
        )
    else:
        file_format = FileFormat.file_format_from_str(file_format)
        if isinstance(file_format, DtlError):
...
class JdbcDatastoreDef(DatastoreDef):
    ...
    def __init__(
        ...
        if root_table is None and view_query is None:
            raise ValueError(
                "Either root_table or view_query must be specified for a JDBC datasource"
            )
        if root_table is not None and view_query is not None:
            raise ValueError(
                "Both root_table and view_query cannot be specified together for a JDBC datastore"
            )
        if post_processing_query is not None and root_table is None:
            raise ValueError(
                "root_table has to be defined in order to use post_processing_query"
            )
...
def _jdbc_datastore_def_from_payload(d: dict) -> Union[DtlError, JdbcDatastoreDef]:
    ...
    if root_table is None and view_query is None:
        return DtlError(
            "'rootTable' or 'viewQuery' needs to be defined in a Jdbc definition"
        )
    if root_table is not None and view_query is not None:
        return DtlError(
            "Either 'rootTable' or 'viewQuery' has to be defined not both in a Jdbc definition"
        )
...
class KinesisDatastoreDef(DatastoreDef):
    ...
    def _from_payload(d: dict) -> Union[DtlError, "KinesisDatastoreDef"]:
        ...
        return KinesisDatastoreDef(
            stream_field, format_field, shard_attributes, params, datastore_id
        )
...
class KafkaDatastoreDef(DatastoreDef):
    ...
    def _from_payload(d: dict) -> Union[DtlError, "KafkaDatastoreDef"]:
        ...
        return KafkaDatastoreDef(
            topic, format_or_registry, params, resource_id, read_from_timestamp
        )
...
def _http_datastore_def_from_payload(d: dict) -> Union[DtlError, HttpDatastoreDef]:
    ...
    if file_format is None:
        return DtlError(
            "'file_format' needs to be defined in an HttpResource definition"
        )
    else:
        file_format = FileFormat.file_format_from_str(file_format)
        if isinstance(file_format, DtlError):
    ...
    if method_type is None:
        return DtlError(
            "'method_type' needs to be defined in an HttpResource definition"
        )
    else:
...
class WebhookDatastoreDef(DatastoreDef):
    ...
    def _webhook_def_from_payload(d: dict) -> Union[DtlError, "WebhookDatastoreDef"]:
        ...
        if not isinstance(message_format, str):
            return DtlError(
                "'message_format' needs to be defined in a Webhook definition"
            )
        else:
...
class SFTPDatastoreDef(DatastoreDef):
    ...
    def _sftp_def_from_payload(d: dict) -> Union[DtlError, "SFTPDatastoreDef"]:
        ...
        if not isinstance(path, str):
            return DtlError(
                "'path' needs to be specified for SFTP datastore definition"
            )

        file_format = d.get("fileFormat")
        if not isinstance(file_format, str):
            return DtlError(
                "'fileFormat' needs to be specified for SFTP datastore definition"
            )
        else:
            file_format = FileFormat.file_format_from_str(file_format)
        ...
        if decompression is None:
            return DtlError(
                "'decompression' needs to be defined for SFTP datastore definition"
            )
        else:
...
class ElasticSearchDef(DatastoreDef):
    ...
    def _from_payload(cls, json: dict) -> Union[DtlError, "ElasticSearchDef"]:
        ...
        if index is None:
            return DtlError(
                "'index' needs to be defined in an ElasticSearch definition"
            )

        index_id_key = json.get("indexIdKey")
        if index_id_key is None:
            return DtlError(
                "'indexIdKey' needs to be defined in an ElasticSearch definition"
            )
            if not isinstance(index_id_key, list) or not all(
                isinstance(path, str) for path in lst
            ):
        ...
        if type_name is None:
            return DtlError(
                "'docTypeName' needs to be defined in an ElasticSearch definition"
            )
...
def _datastore_def_from_payload(json: dict) -> Union[DtlError, DatastoreDef]:
    ...
    if type_field is None:
        return DtlError(
            "The datastore definition object doesn't have a '%s' property"
    ...
    if parsing_function is None:
        return DtlError(
            "Looks like '%s' datastore is not handled by the SDK" % type_field
        )
...
def _datastore_from_payload(json: dict) -> Union[DtlError, Datastore]:
    ...
    if ads_json is not None:
        ...
        if schema_nodes is not None:
            schema_nodes = _parse_list(SchemaNode._schema_node_from_payload)(
                schema_nodes
            )
            if isinstance(schema_nodes, DtlError):
                return schema_nodes
        ads = AbstractDataSchema._from_payload(
            ads_json, AbstractDataSchema.node_decoder
        )
    tags = json.get("tags")
    if tags is None:
        tags = []
...
class ClassThreshold(RollupStrategy):
    ...
    def _as_payload(self) -> dict:
        payload = {
            "type": "Threshold",
            "thresholds": list(
                map(lambda threshold: threshold._as_payload(), self.threshold_pairs)
            ),
        }
        return payload
