...
class S3Credentials(Credentials):
    ...
    def __init__(
        ...
        if is_client_id_set != is_client_secret_set:
            raise ValueError(
                "`client_id` and `client_secret` must but set, or both can't be set"
            )
        ...
        if is_session_timeout_in_hours_set and (not is_role_arn_set):
            raise ValueError(
                "`role_arn` must be set if `session_timeout_in_hours` specifies a value"
            )
    ...
    def __repr__(self):
        ...
        if hasattr(self, "client_id") and hasattr(self, "client_secret"):
            repr_str = (
                repr_str + f"client_id= {self.client_id!r}, client_secret= ****, "
            )
        ...
        if hasattr(self, "role_arn"):
            ...
            if hasattr(self, "session_timeout_in_hours"):
                repr_str = (
                    repr_str
                    + f", session_timeout_in_hours= {self.session_timeout_in_hours!r}"
                )
        repr_str = repr_str + ") "
        return repr_str
    ...
    def _as_payload(self) -> dict:
        ...
        if hasattr(self, "role_arn"):
            ...
            if hasattr(self, "session_timeout_in_hours"):
                ...
                ] = self._validate_and_convert_to_seconds_session_timeout(
    ...
    def _validate_and_convert_to_seconds_session_timeout(
        ...
        if not 1 <= session_timeout_in_hours <= 12:
            raise DtlError(
                "The `session_timeout_in_hours` should be between 1 and 12 hours"
            )
        else:
...
class AmazonVendorCentral(Credentials):
    ...
    def __init__(
        self, client_id: str, client_secret: str, mws_auth_token: str, seller_id: str
    ):
...
class AzureSASCredentials(Credentials):
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(sas_token= ****,"
