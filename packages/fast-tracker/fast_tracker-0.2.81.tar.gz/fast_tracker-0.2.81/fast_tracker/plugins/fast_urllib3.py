...
def install():
    ...
    def _fast_request(this: RequestMethods, method, url, fields=None, headers=None, **urlopen_kw):
        ...
        context = get_context()
        functions.log("fast_urllib path:%s", url_param.path)
        with context.new_exit_span(op=url_param.path.split("?")[0] or "/", peer=url_param.netloc) as span:
            carrier = span.inject()
            functions.log("flb3 carrier val:%s",carrier.val)
