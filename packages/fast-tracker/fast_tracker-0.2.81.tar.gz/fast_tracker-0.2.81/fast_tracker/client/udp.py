...
class UdpTraceSegmentReportService(TraceSegmentReportService):
        ...
        try:
            ...
            for segment in generator:
                for span in segment.spans:
                    ...
                    # res = udp_sock.sendto(data, ("fast-collector", self.udp_port))
                    # functions.log("sended udp socket ip: %s, port: %d", self.udp_ip, self.udp_port)
                    functions.log(": %r, result is: %r", data, res)
    ...
    def _get_data(self, segment, span):
        ...
        report_struct.service_instance = config.service_name + "|" + str(os.getpid())+"|"+socket.gethostname()
        report_struct.service_version = config.get_service_version()
