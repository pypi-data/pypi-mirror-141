...
def logisticregression(x,y):
    ...
    def plotting(xtrain,ytrain):
        if best_params[best_index]['solver'] == 'saga':
            plt.title('LogisticRgression Decision Region Boundary')
            plot_decision_regions(xtrain,ytrain,clf=scv_model1,zoom_factor=2)
            plt.show()
        elif best_params[best_index]['solver'] == 'liblinear':
            plt.title('LogisticRgression Decision Region Boundary')
            plot_decision_regions(xtrain,ytrain,clf=scv_model2,zoom_factor=2)
            plt.show()
        else:
            plt.title('LogisticRgression Decision Region Boundary')
            plot_decision_regions(xtrain,ytrain,clf=scv_model3,zoom_factor=2)
            plt.show()
