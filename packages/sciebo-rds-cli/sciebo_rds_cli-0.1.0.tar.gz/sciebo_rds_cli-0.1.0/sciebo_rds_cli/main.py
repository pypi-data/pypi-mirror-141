...
def execute_helm(values_file, install=False):
    if install:
    ...
    error_code = os.system(
        f"helm upgrade -i sciebo-rds sciebo-rds/all --values {values_file}"
    )
    if error_code > 0:
...
def init(self_signed, overwrite_values):
    """
...
def checks():
    ...
    if (
        ...
        click.echo(
            f"{values_file_path} not valid. Helm founds error with it.", err=True
        )
        error_found = True
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="A convenient parameter, which needs helm installed. It runs all needed helm commands to install sciebo-rds in your current kubectl context after configuration. Helm upgrades should not use this parameter. Please use `helm upgrade` for this.",
)
@click.option(
    "-c",
    "--config",
...
def install(force_kubectl, helm_install, values_file, file):
    ...
    if helm_install:
        execute_helm(values_file, install=True)
