...
from ._util._utils import _get_name_in_mapping, _get_parameter_static_value, is_float_convertible, \
    ...
    _dumps_raw_json, basic_python_type, _is_json_string_convertible, is_hyper_parameter
...
class ComponentValidator(object):
    ...
    def validate_component_inputs(workspace, provided_inputs,
        ...
        def is_pipeline_parameter(input):
            ...
            """
            for pipeline_parameter in pipeline_parameters.values():
                if type(input) != type(pipeline_parameter):
                    continue
                if type(input) in basic_python_type:
                    if input == pipeline_parameter:
                        return True
                else:
                    if id(input) == id(pipeline_parameter):
                        return True
            return False
    ...
    def validate_component_parameters(provided_parameters,
        ...
        def _is_conditional_optional(parameter, provided_parameters):
            ...
            # use for to iterate every item to avoid comparison with PipelineParameter
            for enabled_by_value in enabled_by_values:
                if type(provided_value) != type(enabled_by_value):
                    continue
                if type(provided_value) in basic_python_type:
                    if provided_value == enabled_by_value:
                        return False
                else:
                    if id(provided_value) == id(enabled_by_value):
                        return False
            return True
        ...
        for parameter in interface_parameters:
            ...
            if (provided_param_value is None) or (
                    parameter.parameter_type == '3' and type(provided_param_value) is str
