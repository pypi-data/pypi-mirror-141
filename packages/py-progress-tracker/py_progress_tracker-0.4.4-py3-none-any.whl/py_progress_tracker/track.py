...
The behavior of the benchmarks can be custumized with the following environment variables:
    ...
    def inner(main):
        ...
        for target in targets:
            ...
            for i in range(1, samples_for_target + 1):
                ...
                try:
                    class Subprocess:
                        def __call__(self, output, parameters):
                            main(**parameters)
                            with open(output, "w") as f:
                                json.dump([METRICS, MEASUREMENTS, ALERTS], f)

                    channel = tempfile.NamedTemporaryFile()
                    process = multiprocessing.Process(
                        name="(Sampler)",
                        target=Subprocess(),
                        args=(channel.name, parameters),
                    )
                    ...
                    with open(channel.name, "r") as f:
                        new_metrics, new_measurements, new_alerts = json.load(f)
