...
def safe_update_state(experiment: Experiment, state: str):
    ...
    try:
        ...
        print(f'Encountered error when trying to update the state of {experiment} to {state}: {exception}')


def run_network(network_id: str, source_data: Mapping, sink_data: Mapping[str, str], label: str, fastr_home: Optional[str], config: Dict[str, Any]):
...
def fastr(experiment: Experiment,
          action: Action,
          config: Dict[str, Any],
          network_id: str,
          source_mapping: Mapping[str, str],
    ...
    logging.debug(f"Starting callback for action_uri: {action}")
    xnat_server = experiment.session.external_systems[xnat_external_system_name].url.rstrip('/')
    logging.debug(f"Using xnat: {xnat_server}")

    # Set processing state
    safe_update_state(experiment, process_state)
    ...
    try:
        ...
        result, log_data = run_network(network_id, source_data, sink_data, experiment_label, fastr_home, config)
    except Exception as excp:
        # If something goes wrong, transition to failed state
        safe_update_state(experiment, failed_state)
        raise
    ...
    if result:
        print('SUCCESS')
        safe_update_state(experiment, done_state)
    else:
        print('FAILED')
        safe_update_state(experiment, failed_state)
