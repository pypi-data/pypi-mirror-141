...
def task_callback(celery_self, callback_data: str, action_url: str, config: Dict[str, Any]):
    try:
        master_callback(callback_data, action_url, config)
    except Exception as exc:
        celery_self.retry(exc=exc)
        

def celery_callback(callback_data: str, action_url: str, config=None):
    callback_data_info = yaml.safe_load(callback_data)
    callback_function = callback_data_info['function']
    callback_queue = callback_data_info.get('queue', 'celery')
    #callback_tag = callback_data_info.get('tag', 'default')
    ...
    current_app.logger.error(f'Using celery callback queue: {callback_queue}')
    result = task.apply_async((callback_data, action_url, config), queue=callback_queue)
