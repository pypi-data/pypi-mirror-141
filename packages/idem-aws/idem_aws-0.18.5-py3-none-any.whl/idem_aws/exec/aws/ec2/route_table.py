...
async def update_associations_routes(
    ...
    if new_routes is []:
        ...
        routes_to_modify = (
            hub.tool.aws.route_table_utils.get_route_table_routes_modifications(
        ...
        if routes_to_delete:
            for route_to_delete in routes_to_delete:
                ...
                if not ret.get("result"):
                    ...
                    result["result"] = False
                    return result
            final_comment = final_comment + f"Deleted Routes: {routes_to_delete}, "
        if routes_to_replace:
            for route_to_replace in routes_to_replace:
                ...
                if not ret.get("result"):
                    ...
                    result["result"] = False
                    return result
            final_comment = final_comment + f"Replace Routes: {routes_to_replace}, "
        if routes_to_add:
            for route_to_add in routes_to_add:
                ...
                if not ret.get("result"):
                    ...
                    result["result"] = False
                    return result
            final_comment = final_comment + f"Added Routes: {routes_to_add}, "
    ...
    if new_associations is not None:
        ...
        associations_to_modify = (
            hub.tool.aws.route_table_utils.get_route_table_associations_modifications(
                old_associations, new_associations
            )
        )
        if not associations_to_modify["result"]:
            result["comment"] = associations_to_modify["comment"]
            result["result"] = True
            return result
        ...
        if associations_to_delete:
            for association_to_delete in associations_to_delete:
                ...
                if not ret.get("result"):
                    ...
                    return result
            final_comment = (
                final_comment + f"Deleted Associations: {associations_to_delete}, "
            )
        if associations_to_add:
            for association_to_add in associations_to_add:
                ...
                if not ret.get("result"):
                    ...
                    return result
            final_comment = (
                final_comment + f"Added Associations: {associations_to_add}, "
            )
    if new_propagating_vgws is not None:
        ...
        propagating_vgws_to_modify = hub.tool.aws.route_table_utils.get_route_table_propagating_vgws_modifications(
        ...
        if propagating_vgws_to_add:
            for propagating_vgw_to_add in propagating_vgws_to_add:
                ret = await hub.exec.boto3.client.ec2.enable_vgw_route_propagation(
                    ctx, RouteTableId=route_table_id, GatewayId=propagating_vgw_to_add
                )
                if not ret.get("result"):
                    result["comment"] = ret["comment"]
                    result["result"] = False
    ...
    result["comment"] = f"Updated route table {route_table_id}, {final_comment}"
    return result
