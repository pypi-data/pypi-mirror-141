...
def create_users_table_model(database):
    ...
    for row in users_list:
        ...
        table_model.appendRow([user, ip, port, time])
    return table_model

def create_history_table_model(database):
    users_history = database.login_history()
    table_model = QStandardItemModel()
    table_model.setHorizontalHeaderLabels(
        ['Имя Клиента', 'Последний раз входил', 'IP Адрес', 'Порт',])
    for row in users_history:
        user, last_login, ip, port = row
        user = QStandardItem(user)
        user.setEditable(False)
        ...
        port = QStandardItem(str(port))
        port.setEditable(False)
        table_model.appendRow([user, last_login, ip, port])
    return table_model
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # Кнопка вывести историю подключений клиентов
