...
def date_features(df):
    ...
    df.loc[:, 'day_of_year'] = int(df.index.dayofyear)
    df.loc[:, 'month'] = int(df.index.month)
    df.loc[:, 'day_of_week'] = int(df.index.day)
    df.loc[:, 'hour'] = int(df.index.hour)
    return df
    ...
    def cont_cat_split(self):
        ...
        try:
            if is_pandas(self.data):
                ...
                self.cont = self.data.drop(cat_cols)
