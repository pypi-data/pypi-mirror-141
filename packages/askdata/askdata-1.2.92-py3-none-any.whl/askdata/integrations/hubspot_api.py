...
def get_hubspot_contact(API_KEY, start_date):
    ...
    while i >0 :
        ...
        for dd in d1:
            ...
            start = (date.today() - start_date).days
            ...
            end = (date.today() - end_date).days
            
            end_date = (dt.today()-td(days=end)).isoformat()
            
            
            try:
                end_date = dt.strptime(end_date,'%Y-%m-%dT%H:%M:%S.%f')
            except:
            ...
            if  upd >= start_date and upd <= end_date:
                if y%250 == 0:
                    print ("Iteration num.", y)
                y= y+ 1
                try:
