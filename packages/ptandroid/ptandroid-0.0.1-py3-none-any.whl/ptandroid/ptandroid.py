...
class ptandroid:
    ...
    def check_dependencies(self):
        ...
        try:
            ...
            ptmisclib.end_error("Androguard not found (required for parsing apk), please install by running: 'pip install androguard'", self.json_no, self.ptjsonlib, self.use_json)
        try:
            ...
            ptmisclib.end_error(f"Cannot execute nor set execution privileges for apktool, please run script as root or manually run 'sudo chmod +x {self.path2apktool}'", self.json_no, self.ptjsonlib, self.use_json)
    ...
    def print_axml(self, axml_content):
        ptmisclib.ptprint(ptmisclib.out_ifnot("AndroidManifest.xml", "INFO", condition=self.use_json, colortext=True))
        while axml_content.endswith("\n"):
            axml_content = axml_content[:-1]

        ptmisclib.ptprint(ptmisclib.out_ifnot(axml_content, "", self.use_json))
    ...
    def _decompile_apk(self, dir_path):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Decompiling apk..", "INFO", self.use_json), f"\r")
        ...
        for d in result_list:
            for key, value in d.items():
                ...
                if isinstance(value, (str, bool)) or value is None:
                    ptmisclib.ptprint(ptmisclib.out_ifnot(f"{key}: {str(value)}", "", self.use_json))
                if isinstance(value, list):
                    if nested or (not nested and key.upper() != d['test_name'].upper().replace("(","").replace(")","")):
                        ptmisclib.ptprint(ptmisclib.out_ifnot(f"{key}:" ,"", self.use_json), end="\n")
                    if not value:
                        ptmisclib.ptprint(ptmisclib.out_ifnot(f"{spacing}None" ,"", self.use_json))
                        continue
                    elif isinstance(value[0], str):
                        ptmisclib.ptprint(f"{spacing}"+ptmisclib.out_ifnot(f'\n{spacing}'.join(value) ,"", self.use_json))
