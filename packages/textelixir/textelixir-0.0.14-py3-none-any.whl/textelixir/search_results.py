...
class SearchResults:
        ...
        for block_num, chunk in enumerate(self.elixir):
            ...
            if isinstance(search_word, str):
                ...
                for word in found_words.to_dict('index'):
                    find_index = word-(block_num*self.chunk_size)
    ...
    def vocab_distribution(self, **kwargs):
        ...
        return VocabDist(self.filename, group_by, self.results_indices, self.punct_pos, self.search_string, self.chunk_size)
    ...
    def collocates(self, before=5, after=5, group_by='lemma_pos', mi_threshold=3, sample_size_threshold=2):
        ...
        collocates = Collocates(self.filename, self.results_indices, before, after, self.word_count, group_by, mi_threshold=mi_threshold, sample_size_threshold=sample_size_threshold, search_string=self.search_string, chunk_size=self.chunk_size)
    ...
    def calculate_collocate_totals(self, samples):
        ...
        totals = SearchResults(self.filename, words, self.word_count, punct_pos=self.punct_pos, verbose=False, text_filter=None, regex=False, chunk_size=self.chunk_size).results_totals
        return totals

    ### SENTENCES HANDLER
    def sentences(self, group_by='text'):
        return Sentences(self.filename, self.results_indices, group_by=group_by, search_string=self.search_string, chunk_size=self.chunk_size)
