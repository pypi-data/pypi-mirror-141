...
def init():
    for key in keys(killed):
        val = get(killed, key, None)
        if val:
            e = Event()
            e.txt = ""
            e.rest = key
            sec = seconds(val)
            repeater = Repeater(sec, stat, e, name=" ".join(key.split()[1:]))
            repeater.start()
...
def stat(e):
    ...
    if needed:
        ...
        txt = "%s: %s (%s) " % (" ".join(name.split()[1:]), nrtimes, elapsed(needed))
        Bus.announce(txt)


def stats(event):
    txt = ""
    for name in sorted(killed, key=lambda x: seconds(nr(x))):
        needed = seconds(nr(name))
        delta = time.time() - starttime
        nrtimes = int(delta/needed)
        txt += "%s: %s (%s) " % (" ".join(name.split()[1:]), nrtimes, elapsed(needed))
    event.reply(txt.rstrip())
