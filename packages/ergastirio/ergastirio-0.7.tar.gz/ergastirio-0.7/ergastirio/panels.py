...
class acquisition_control():
    ...
    def press_enter_edit_RefreshTime(self):
        ...
        self.edit_RefreshTime.setText(f"{self.experiment.refresh_time:.3f}") #In case refresh_time is not valid, this instruction will restore the displayed value to its previous (valid) value
        return True
    ...
    def click_radio_global_instrument(self):
        if self.radio_TriggerGlobal.isChecked():
            self.experiment.triggered_acquisition = False
