...
class SdpUtils:
    ...
    def csv_to_raw(self, path, chunksize=700, separator=';', verbose=False, check_duplicates=False, id_col=None):
        ...
        table_name = self.get_csv_name(path)
        
        if check_duplicates and id_col == None:
            raise Exc.InvalidIdCol

        df_generator = pd.read_csv(path, sep=separator, header=0, skipinitialspace=True, keep_default_na=True, infer_datetime_format=True, keep_date_col=True, encoding='utf-8', chunksize=chunksize)
        
        if verbose:
            print(f'Uploading {table_name} to raw zone..')
        
        uploaded = 0
        print(f'Uploaded: {uploaded}', end='\r')
