...
class mtse:
    ...
    def build_model(self, encoder='mtan', model_type='regression', regressor=None, classifier=None, classif_out=2, seq_encoder=None, nhidden=64, embed_time=64, n_heads=1, learn_emb=True, optim='default', sched='default', lr=0.01, checkpoint_path=None, early_stop=0, min_delta=0):
        ...
        self.optimizer = torch.optim.Adam(self.params, lr=lr) if optim == 'default' else optim
        if optim != 'default':
            self.optimizer.add_param_group({'params': self.params})
    ...
    def train_and_predict(self, cuda_empty_cache=True, lossf='rmse', plot_results=True, x_start=10, argmax=False, 
        ...
        if plot_results:
            print(f'\n### Plotting results ###\nfigure saved as `results_{self.experiment_id}.png` and as `results_{self.experiment_id}.pickle` \n')        
    ...
    def predict_from_pretrained(self, checkpoint='best'):
        print('\n### Predicting the target on the test set ###')
        if checkpoint == 'best':
            try:
                return predict(self.test_loader, self.device, self.best_model, model_type=self.model_type, dim=self.n_ts)
            except:
                raise ValueError('To predict from the best model, you need to train the model on this instance first.')
            ...
            return predict(self.test_loader, self.device, pt_encoder, model_type=self.model_type, dim=self.n_ts)
    ...
    def encode_ts(self, embed_pandas=True, data_to_embed='test', has_label=False, checkpoint='best'):
        ...
        if checkpoint == 'best':
            try:
                ...
                raise ValueError('To encode from the best model, you need to train the model on this instance first.')
        elif checkpoint == 'last':
            print('Caution: if no training has been performed, the prediction will be random and irrelevant.')
            try:
                pt_encoder = self.ts_encoder
