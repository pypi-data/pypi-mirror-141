...
class mtse:
    ...
    def load_data(self, train_data=None, val_data=None, test_data=None, order_data=True, reduce_data=True, norm=None, batch_size=64, shuffle=False):
        ...
        self.test_loader = DataLoader(mtan_Dataset(test_data, test_data=True, order_data=order_data, reduce_data=reduce_data, norm=norm), batch_size=batch_size, shuffle=False) if test_data is not None else None
        print('...done...')
    ...
    def train(self, lossf='rmse', plot_results=True, save_plot=True, x_start=10, argmax=False, CEweights=None,
              predict_strategy=None, save_strategy='best', val_loss_threshold=100., n_iters=100, val_loss_fn=None):
        """
        Parameters
        ----------
        ...
        val_loss_fn : NoneType or Callable, optional
            if None, same as the loss function; if a function, it is used to compute the validation loss
        ...
        if CEweights is not None:
            criterion = nn.CrossEntropyLoss(torch.tensor(CEweights).to(self.device).float())

        val_loss_fn = val_loss_fn if val_loss_fn is not None else criterion
        ...
        for itr in range(1, n_iters + 1):        
            ...
            val_loss = evaluate_model(self.ts_encoder, self.val_loader, dim=self.n_ts, loss=val_loss_fn, argmax=argmax, device=self.device)
            ...
            if save_strategy == 'best' and val_loss == best_val_loss and val_loss <= val_loss_threshold:
                ...
                self.best_model = deepcopy(self.ts_encoder)
    ...
    def predict(self, checkpoint='best'):
        ...
        if checkpoint == 'best':
            ...
            try:
                ...
                pt_model = torch.load(checkpoint, map_location=self.device)
            except:
