...
class Bot:
    ...
    def load_commands(self, **kwargs):
        ...
        if "cog_name" in kwargs: cog_name = kwargs["cog_name"]
        else: cog_name = lib_path.split(".")[-1]
        ...
        # get all the functions inside the class's register function
        for func in inspect.getmembers(class_instance, inspect.isfunction):
            if func[0] == "register":
                print(func[1])

        class_instance.register()

        cog_obj = cog.Cog(cog_name, commands_class, lib_path, commands)
