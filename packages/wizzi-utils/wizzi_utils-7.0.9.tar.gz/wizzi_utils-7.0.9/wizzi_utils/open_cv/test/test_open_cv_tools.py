...
def invert_image_test():
    ...
    cv_gray = cvt.BGR_img_to_gray(cv_img)
...
def all_color_maps_test():
    ...
    cvt.display_open_cv_images(
        ...
        loc='cc',
        resize=0.3,
        grid=(3, 8),  # 24 total with the originals
        header=None
    )
    cv2.destroyAllWindows()
    return
...
def get_roi_2dpoints_test():
    ...
    d2points = cvt.get_roi_2dpoints(cv_img, resize=1.6, mark_color='r', add_center_text=True)  # enlarge to 160%
...
def get_named_roi_2dpoints_test():
    ...
    colors_points = cvt.get_named_roi_2dpoints(cv_img, names_colors=names_colors, resize=1.6,
                                               add_center_text=True)  # enlarge to 160%
