...
class Py3Object(Base):
    ...
    def instantiate(self, args: Vector) -> Py3Inst:
        ...
        shortcuts.ASSERT_IS_CHILD_OF(
            args,
            Vector,
            message='Py3Object.instantiate: the first argument needs to be Vector'
        )
        ...
        for idx, arg in enumerate(args.items()):
            ...
            if issubclass(arg.__class__, (Collection,)):
                if isinstance(arg, HashMap):
                    ...
                    for key, value in arg.reference().values():
                        assert isinstance(key, (String, Keyword)), (
                            'Py3Object.instantiate(): kw should be String/Keyword'
                        )
                        assert not issubclass(value.__class__, (Collection,)),   (
                            'Py3Object.instantiate(): Collection is not supported'
                        )
                        _kwargs.update({   key.reference(): value.reference()   })
                    break  # < prevent from adding HashMap to positional arguments
                raise AssertionError(  'Py3Object.instantiate(): no Collection'  )
