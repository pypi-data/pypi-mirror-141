...
def auto_execute(sqlshell, data, *args, **kwargs):
    ...
    if ident_id:
        ...
        ident_id = create_new_ident
    if auto_id:
        update_car(sqlshell, car_number, ident_id, active)
    else:
...
def update_car(sql_shell, car_number, ident_id, active):
    command = "UPDATE auto SET identifier={}, active WHERE car_number='{}'"
    command = command.format(ident_id, active, car_number)
    sql_shell.try_execute(command)

def fix_ident_to_auto(sql_shell, auto_id, ident_id):
    command = "UPDATE auto SET identifier={} WHERE id={}"
    command = command.format(auto_id, ident_id)
    return sql_shell.try_execute(command)
