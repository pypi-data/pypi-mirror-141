...
class IDACRequest:
    ...
    async def check_controller_auth(self) -> None:
        ...
        logger.debug(f"Checking auth. Type is {self.controller.auth_type}")
        if self.controller.auth_type == IDACAuthType.DCLOUD_SESSION:
    ...
    async def create(
        ...
        if method == "POST":
            ...
            kwargs = {"json": self.session_data.dict(exclude_none=True)}
            headers.update({"Content-Type": "application/json"})
        else:
            # put data in query string if GET
            kwargs = {"params": self.session_data.dict(exclude_none=True)}
