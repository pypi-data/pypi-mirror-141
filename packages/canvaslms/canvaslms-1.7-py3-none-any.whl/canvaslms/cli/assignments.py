...
def process_assignment_option(canvas, args):
  ...
  for course in course_list:
    all_assignments = list(list_assignments([course], ungraded=args.ungraded))
    assignment_groups = filter_assignment_groups(course, args.assignment_group)
...
def filter_assignments(assaignments_containers, regex):
  """Returns all assignments from assignments_container whose
  title matches regex"""
  p = re.compile(regex)
  for assignment in list_assignments(assignments_containers):
