...
yaml_cfg_loader = Callable[[Arg(str, 'filename'),
                            NamedArg(bool, 'no_exist_ok'),
                            NamedArg(bool, 'local_only')],
                            dict[str, str]]
...
def setup(etc_dir: Path, load_cfg_from_yaml: yaml_cfg_loader) -> None:
    """Sets up necessary directories for Conduit, and reads in secrets.

    The `etc/auth/` and `etc/auth/secrets/` directores are created here.

    Args:
        etc_dir (Path): A `Path` to the etc directory.
        load_cfg_from_yaml (Callable): Function that can load in a YAML config file, used
            to read in the secrets file.
    """
...
class AuthNotSetupExeception(Exception):
