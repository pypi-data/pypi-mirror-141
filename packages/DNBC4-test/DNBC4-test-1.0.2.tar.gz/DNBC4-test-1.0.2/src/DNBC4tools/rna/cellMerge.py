...
raw_beads_stat = pd.read_table(os.path.join(indir,'../02.cDNAAnno/%s.beads_stat.txt'%name),sep = '\t')
...
barcodeTranslate['Count'] = barcodeTranslate['CELL'].str.split('_N',expand=True)[1]
figtable = barcodeTranslate.Count.value_counts()
...
#save_as_pdf_pages(p,filename='%s/%s_CellNumber_merge.pdf'%(indir,name))
p.save(filename = '%s/%s_CellNumber_merge.png'%(indir,name))
p.save(filename = '%s/%s_CellNumber_merge.pdf'%(indir,name))