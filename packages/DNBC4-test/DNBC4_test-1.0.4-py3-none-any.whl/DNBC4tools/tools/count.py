...
from .utils import str_mkdir,change_ld_path,start_print_cmd
...
class Count:
    ...
    def run(self):
        ...
        combineBeads_cmd = 'python %s/rna/combinedListOfBeads.py %s/02.count/Similarity.droplet.filtered.csv %s/02.count/%s_combined_list.txt'\
            %(_root_dir,self.outdir,self.outdir,self.name)
        CellMerge_cmd = 'python %s/rna/cellMerge.py --indir %s/02.count --name %s'\
            %(_root_dir,self.outdir,self.name)
        ...
        saturation_cmd = 'python %s/rna/saturation.py --inbam %s/02.count/anno_decon.bam --outdir %s/02.count --quality 20 --threads %s'\
            %(_root_dir,self.outdir,self.outdir,self.thread)
        ...
        start_print_cmd(saturation_cmd)
