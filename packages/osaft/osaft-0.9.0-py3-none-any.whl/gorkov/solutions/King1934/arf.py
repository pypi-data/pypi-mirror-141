...
from gorkov.core.functions import pi, sin
...
from gorkov.solutions.King1934.scattering import ScatteringField
...
class ARF(ScatteringField, BaseARF):
    ...
    def __init__(
        ...
        p_0: float, wave_type: WaveTypes, position: Optional[float] = None,
        N_max: int = 5,
    ...
    def acoustic_radiation_force(self) -> float:
        ...
        elif self.wave_type == WaveTypes.STANDING:
    ...
    def _compute_arf_traveling(self) -> float:
        ...
        """
        first = 1 / self.HH_n(0)
        second = 2 / self.HH_n(1)
        second *= (self.alpha ** 2 - 3 * (1 - 1 / self.rho_tilde)) ** 2
        second /= self.alpha ** 8

        def higher():
            def term(order):
                out = (order + 1) / self.HH_n(order)
    ...
    def _compute_arf_standing(self) -> float:
        ...
        Radiation Pressure on Spheres"
        """
        first = self.FFGG(0) / (self.alpha * self.HH_n(0))
        second = - 2 / self.alpha ** 5 * self.FFGG(1) / self.HH_n(1)
        ...
        def higher():
            def term(order):
                ...
                out *= self.FFGG(order) / self.HH_n(order)
