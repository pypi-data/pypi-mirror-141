...
from gorkov.core.generic_types import t_fluid
...
class WaveTypes(DocEnum):
    """Supported Wavetypes for the acoustic backgorundfield
    """
...
class BackgroundField:
    ...
    def __init__(
        ...
        if position is None and wave_type == WaveTypes.STANDING:
            raise ValueError(
    ...
    @property
    def f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.f`
        """
        return self.fluid.f
    ...
    def omega(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.omega`
        """
    ...
    @property
    def c_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.c_f`
        """
    ...
    def rho_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.rho_f`
        """
    ...
    def eta_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscousFluid.eta_f`
        """
        try:
    ...
    def zeta_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscousFluid.zeta_f`
        """
        try:
    ...
    def eta_p(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscoelasticFluid.eta_p`
        """
        try:
    ...
    def zeta_p(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscoelasticFluid.zeta_p`
        """
        try:
    ...
    def lambda_M(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscoelasticFluid.lambda_M`
        """
        try:
    ...
    def lambda_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.lambda_f`
        """
        return self.fluid.lambda_f

    @property
    def k_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.InviscidFluid.k_f`
        """
        return self.fluid.k_f

    @property
    def k_v(self) -> complex:
        """ wrapper for :attr:`gorkov.core.fluids.ViscousFluid.k_v`
        """
        return self.fluid.k_v

    @property
    def kappa_f(self) -> float:
        """ wrapper for :attr:`gorkov.core.fluids.ViscousFluid.kappa_f`
        """
    ...
    def _compute_A_in(self, N: int) -> None:
        ...
        if self.wave_type == WaveTypes.TRAVELLING:
            self.__compute_A_in_travelling(N, N_old)
        else:
            self.__compute_A_in_standing(N, N_old)

    def __compute_A_in_travelling(self, N: int, N_old: int) -> None:
        for n in full_range(N_old, N):
            self._A_in.value.append(self.A * (2 * n + 1) * 1j ** n)

    def __compute_A_in_standing(self, N: int, N_old: int) -> None:
        for n in full_range(N_old, N):
            new = exp(1j * self.abs_pos * self.k_f)
            new += (-1) ** n * exp(-1j * self.abs_pos * self.k_f)

            new *= self.A / 2 * (2 * n + 1) * 1j ** n
            self._A_in.value.append(new)
