...
from gorkov.core.backgroundfields import (
    BackgroundField,
    WaveTypes,
    WrongWaveTypeError,
)
from gorkov.core.fluids import InviscidFluid, ViscoelasticFluid, ViscousFluid
from gorkov.core.functions import exp, pi
from gorkov.tests.basetest import BaseTest


class TestBackgroundFieldWrappers(BaseTest):
    ...
    def setUp(self) -> None:
        ...
        self.list_cls = [
            self.cls_inviscid, self.cls_viscous,
...
class TestViscousBackgroundField(BaseTest):
    ...
    def setUp(self) -> None:
        ...
        self.cls = BackgroundField(
            self.fluid_cls, self.p_0,
            self.wave_type, self.position,
        )
...
# -----------------------------------------------------------------------------
    ...
    def compute_A_in(self, n) -> complex:
        ...
        if wave_type == WaveTypes.TRAVELLING:
            return A * (2 * n + 1) * 1j ** n
        elif wave_type == WaveTypes.STANDING:
...
# -----------------------------------------------------------------------------
    ...
    def test_wave_type(self):
        self.assertRaises(
            ValueError, BackgroundField,
            self.fluid_cls, self.p_0, WaveTypes.STANDING,
        )
    ...
    def test_A_in(self) -> None:
        for n in range(4, 0, -1):
            self.do_testing(
                func_1=self.compute_A_in, args_1=n,
                func_2=self.cls.A_in, args_2=n,
            )
    ...
    def test_I(self) -> None:
        ...
        self._wave_type.list_of_values = [WaveTypes.TRAVELLING]
