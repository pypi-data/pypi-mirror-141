...
from gorkov import Gorkov1962, King1934, WaveTypes, Yosioka1955
from gorkov.core.backgroundfields import WrongWaveTypeError
from gorkov.core.functions import SpecialFunctions as Sp
from gorkov.core.functions import conj, pi, sin
from gorkov.tests.basetest import BaseTest
from gorkov.tests.basetest_arf import HelperCompareARF
...
class TestARF(BaseTest):
    ...
    def acoustic_radiation_force(self):
        a = self.R_0
        rho = self.rho_f
        lamb = self.cls.lambda_rho
        k = self.cls.k_f
        ...
        for n in range(self.cls.N_max):
            ...
            s += s1 + s2 + s3 + s4
        if self.cls.wave_type == WaveTypes.STANDING:
            return -s
        return s
    ...
    def test_value_error_F(self):
        # Small Particle
        self.cls.bubble_solution = True
        self.cls.wave_type = WaveTypes.TRAVELLING
