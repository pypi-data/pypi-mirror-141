...
def find_mps(model, subset=set(), method='iterative', graph=True, random=False,
             ...
             verbose=False, export=False, output_dir=""):
    ...
    if export:
        ...
        save_set(subset, os.path.join(output_dir, '_'.join([model.ModelName, ts, 'subset.txt'])))
    ...
    if export:
        ...
        save_bounds(bounds, os.path.join(output_dir, '_'.join([model.ModelName, ts, 'bounds.csv'])))
    ...
    if export:
        ...
        log = open(os.path.join(output_dir,'_'.join([model.ModelName, ts, suffix])), 'w')
    ...
    if mps:
        mps = set(frozenset(v.varName.replace('_rev', '') for v in mp)
                  for mp in mps)
    if mcs:
        mcs = set(frozenset(v.varName.replace('_rev', '') for v in mc)
                  for mc in mcs)
    ...
    if mps:
        ...
        if export:
            ...
            if random:
                suffix = 'random_' + suffix
            save_sets(mps, os.path.join(output_dir, '_'.join([model.ModelName, ts, suffix])))
    ...
    if mcs:
        ...
        if export:
            ...
            if random:
                suffix = 'random_' + suffix
            save_sets(mcs, os.path.join(output_dir, '_'.join([model.ModelName, ts, suffix])))
    ...
    if complete and export:
        ...
        if path in os.listdir('.'):
            os.rename(path, os.path.join(output_dir, path.replace('incomplete', 'complete')))
