...
class manson(object):
    ...
    def __init__(self, port: str, mode: int = 232, channel: int = -1, sp=None):
        ...
        if mode not in self.supported_modes:
            msg = "wrong device type , supported devices: "
    ...
    def txrx(self, send, arg=""):
        ...
        self.sp.write(bytearray(cmd.encode()))
        out = self.readline()
    ...
    def init_serial(self):
        ...
        if max_values in self.supported_devices.keys():
            ...
            self.logger.error(f"Unknown device config:\t{max_values}")
    ...
    def set_volts(self, volts):
        ...
        if volts > self.Vmax:
            ...
            self.output_off()
            self.logger.error(str(volts) + "V too low, min 1V -> deactivate output")
        else:
    ...
    def output_on(self, on=True):
        """turn on the DC output of the power suply"""
        self.SOUT("10"[bool(on)])
        time.sleep(0.01)

    def output_off(self):
        """turn off the DC output of the power suply"""
