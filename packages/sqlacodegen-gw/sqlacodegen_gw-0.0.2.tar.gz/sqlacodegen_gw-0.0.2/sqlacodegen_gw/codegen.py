...
class ModelClass(Model):
    ...
    def __init__(self, table, association_tables, inflect_engine, detect_joined):
        ...
        for column in table.columns:
            ...
            # 列名称驼峰化
            column_name = self.fake_tablename_to_classname_(column.name, inflect_engine)
            self._add_attribute(column_name, column)
    ...
    # 添加伪驼峰方法
    @classmethod
    def fake_tablename_to_classname_(cls, tablename, inflect_engine):
        tablename = cls._convert_to_valid_identifier(tablename)
        camel_case_name = ''.join(part[:1].upper() + part[1:] for part in tablename.split('_'))
