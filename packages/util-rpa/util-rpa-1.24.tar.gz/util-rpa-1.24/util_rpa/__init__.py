...
def upload_file_sftp(filename:StrOrPathList, 
    ...
    with paramiko.Transport((credentials["hostname"], puerto)) as transport:
        ...
        with paramiko.SFTPClient.from_transport(transport) as server:
            ...
            try:
                destine_path = PurePosixPath(destine_path)
            except:
            ...
            if type(filename) is list:
                ...
                for i in lista_archivos:
                    ...
                    server.put(i, destine_path / i.name)
            else:
                try:
...
def makedir_sftp(sftp_path:StrOrPath, 
    ...
    with paramiko.Transport((credentials["hostname"], puerto)) as transport:
        ...
        with paramiko.SFTPClient.from_transport(transport) as server:
            ...
            try:
                sftp_path = PurePosixPath(sftp_path)
            except:
                raise Exception("Directorio definido erroneo.")

            log.info("Inicio creacion directorio:" + str(sftp_path))
        ...
        with paramiko.SFTPClient.from_transport(transport) as server:
            ...
            if type(filename) is list:
                ...
                for file in lista_archivos:
                    ...
                    dir_remoto = PurePosixPath(file).parent
                    
                    log.debug("Ubicandose en ruta: " + str(dir_remoto))
                    server.chdir(dir_remoto)
                    
                    destine = destine_path / str(file.name)
                    log.debug("Obteniendo archivo: " + str(PurePosixPath(file)))
                    log.debug("Ruta Destino: " + str(destine))
                    
                    server.get( PurePosixPath(file), destine )
                    
                    if flag_remove:
                        server.remove(PurePosixPath(file))
            else:
                ...
                for file in dir_list:
                    ...
                    if fnmatch.fnmatch(file, filename.name):
                        flag_exists = True
                        filename_remoto = dir_remoto / file
                        log.debug("Obteniendo archivo: " + str(filename_remoto))
