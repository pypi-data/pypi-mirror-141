...
class BaseAction():
    ...
    def check_window_handle_error(self):

        if self.window_handle == 0:
            raise Exception('找不到窗口句柄,请检查窗口配置或者程序问题')
    ...
    def open_client(self):
        ...
        Logger.info('打开客户端')
        Client.open_client(self.config.client_path)
        return 1
    ...
    def input_by_offset(self):
        ...
        self.check_window_handle_error()
    ...
    def __call__(self):
        ...
        if res == 0:
            Logger.error('执行结果错误', exc_info=True, stack_info=True)
