...
class SceneEngineClient(object):
    ...
    def index_rosbag2(self,
        ...
        if folder_uri:
            ...
            folder_name = os.path.normpath(folder_path).split("/")[-1]
        elif folder_path:
            folder_name = os.path.normpath(folder_path).split("/")[-1]
        else:
            raise ValueError("Neither folder_uri or folder_path were specified.  " +
                             "Please provide either folder_uri or folder_path.")

        images_set_id = self.create_set(
            name="{}_{}".format(folder_name, AssetsConstants.IMAGES_ASSET_ID),
            asset_type=AssetsConstants.IMAGES_ASSET_ID,
            is_primary=True
        )
        videos_set_id = self.create_set(
            name="{}_{}".format(folder_name, AssetsConstants.VIDEOS_ASSET_ID),
            asset_type=AssetsConstants.VIDEOS_ASSET_ID,
            is_primary=True
        )
        if deep_analysis:
            objects_set_id = self.create_set(
                name="{}_{}".format(folder_name, AssetsConstants.OBJECTS_ASSET_ID),
        ...
        id_ = self.add_rosbag(folder_uri=folder_uri,
                              folder_path=folder_path)

        logger.info("Calling the index_rosbag endpoint...")
        index_rosbag_json = {"ids": [id_],
                             "search": None,
                             "sets_map": {  # todo: provide handling inside this endpoint if the set doesn't exist
    ...
    def add_rosbag(self,
        ...
        """Upload Rosbag2 onto SceneBox.
           ...
           folder_path:
               ...
               If not None, folder_uri should be None.
           id:
               A unique rosbag ID.  If not listed, ID will become the name of the provided folder.
           session_uid:
               The UID of the session.
           set_id:
               The set id to associate with the ingested rosbag.
           buffered_write:
               Writes the rosbag in a buffered fashion.
        ...
        if folder_path:
            ...
            rosbag_filename = next((file for file in files if
                                    file.split(".")[-1] == "db3"), None)
            metadata_filename = next((file for file in files if
                                      file.split(".")[-1] == "yaml"), None)
            folder_name = os.path.normpath(folder_path).split("/")[-1]
            metdata_path = os.path.join(folder_path, metadata_filename)

            rosbag_metadata["folder_name"] = folder_name
            rosbag_metadata["rosbag_filename"] = rosbag_filename

            id = self.get_asset_manager(
                AssetsConstants.ROSBAGS_ASSET_ID).put_asset(
                file_path=os.path.join(folder_path, rosbag_filename),
                filename=rosbag_filename,
                metadata=rosbag_metadata,
                id=id or folder_name,
                wait_for_completion=not buffered_write,
                buffered_write=buffered_write
            )
            with open(metdata_path) as f:
                metadata_yaml_bytes = f.read()
            meta_object = self.get_asset_manager(
                asset_type=AssetsConstants.ROSBAGS_ASSET_ID).put_file(metadata_yaml_bytes)
            ...
            metadata_filename = next((file for file in files if
                                      file.split(".")[-1] == "yaml"), None)
            rosbag_uri = os.path.join(folder_uri, rosbag_filename)
            metadata_uri = os.path.join(folder_uri, metadata_filename)
            ...
            meta_object = ObjectAccess(uri=metadata_uri)
        else:
            raise ValueError("Either Rosbag file path, uri, url, or bytes must be provided.")

        self.get_asset_manager(
            asset_type=AssetsConstants.ROSBAGS_ASSET_ID).update_metadata(id=id,
                                                                         metadata={"header_object_access":
                                                                                       meta_object.to_dic()})
        return id
