...
def get_format_from_filename(url):
    url = url.lower()
    if url.endswith('csv'):
        return 'csv'
...
async def push_data(ctx, datasource_name, url, connector, sql, mode='append', sql_condition=None, replace_options=None):
    ...
    try:
        ...
        _format = get_format_from_filename(url)
        if parsed.scheme in ('http', 'https'):
...
async def datasource_analyze(ctx, url_or_file, connector):
    ...
    analysis, _ = await _analyze(url_or_file, client, connector=_connector, _format=get_format_from_filename(url_or_file))
...
async def generate_datasource(ctx, connector, filenames, force):
    ...
    for filename in filenames:
        await _generate_datafile(filename, client, connector=_connector, force=force, _format=get_format_from_filename(filename))
