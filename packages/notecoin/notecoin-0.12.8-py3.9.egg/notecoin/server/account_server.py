...
class AccountCoins(RedisConnect):
    def __init__(self, prefix="/account", cache_prefix='okex_account_balance', *args, **kwargs):
        super(AccountCoins, self).__init__(prefix=prefix, cache_prefix=cache_prefix, *args, **kwargs)
        add_api_routes(self)

    @api_route('/update', description="update market tickers")
    def update_value(self, suffix=""):
        data = pd.DataFrame(account_api.get_account().data)
    ...
    def get_value(self, suffix=""):
        return super(AccountCoins, self).get_value(suffix=suffix)
