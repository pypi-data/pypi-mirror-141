...
class DataEntry(tk.Frame):
    ...
    def create_pandas_df(self):
        ...
        # TODO: ------> NEED TO CHECK REMOVAL WAS DONE RIGHT
        try:
        ...
        condition = [page.analysis_parameters['Condition (Optional)'].get()] * len(time_of_spike)#len(self.spikes_indices)
        ...
        notebook = [page.analysis_parameters['Notebook (Required)'].get()] * len(time_of_spike) #len(self.spikes_indices)
        ...
        stim_kind = [page.analysis_parameters['Stimulation Kind (Required)'].get()] * len(time_of_spike) #len(self.spikes_indices)
        stim = [page.analysis_parameters['Stimulation (Required)'].get()] * len(time_of_spike) #len(self.spikes_indices)
        date = [page.analysis_parameters['Date (Required)'].get()] * len(time_of_spike) #len(self.spikes_indices)
        neuron = [page.analysis_parameters['Neuron (Required)'].get()] * len(time_of_spike) #len(self.spikes_indices)
        ...
        df = {"time": time_of_spike, "samples": index_of_spike_in_samples, "signal": signal_value, "isi": isi,
              ...
              "path": [path] * len(time_of_spike), #len(self.spikes_indices),
              "stimulation_kind": stim_kind, 'stimulation': stim,
              "stim_start": [start_time] * len(time_of_spike), #len(self.spikes_indices),
              "stim_end": [end_time] * len(time_of_spike), #len(self.spikes_indices),
    ...
    def spikes_to_bursts(self):
        ...
        #print('Does this fix it??: ', np.arange(len(df['Burst Order'])))
