...
class DataEntry(tk.Frame):
    ...
    def plot_detected_spikes(self):
        ...
        """
        self.burst_annot = page.fig.axes[0].annotate("", xy=(0, 0), xytext=(20, 20), textcoords="offset points",
                                                      bbox=dict(boxstyle="round", fc="b"),
                                                      arrowprops=dict(arrowstyle="->"))
        self.burst_annot.set_visible(False)
        cid2 = page.fig.canvas.mpl_connect('motion_notify_event', self.on_plot_hover)"""
    ...
    def update_spike_annot(self, ind):
        ...
        #print('abs pos: {}, [spike number, burst order] pos {}'.format(self.spikes_df['spike_number'][ind['ind']],
                                               #self.spikes_df['burst_order'][ind['ind']]))
    ...
    def create_pandas_df(self):
        ...
        df = {"time": time_of_spike, "samples": index_of_spike_in_samples, "signal": signal_value, "isi": isi,
              ...
              "path": [path] * len(self.spikes_indices), "stimulation_kind": stim_kind, 'stimulation': stim,
              "stim_start": [start_time] * len(self.spikes_indices), "stim_end": [end_time] * len(self.spikes_indices),
