      ...
      if not component.attested and component.grade != grade:
        ...
        if args.verbose:
          print(f"{course_code} {student}: reported "
            f"{component.component} = {component.grade}.")
      elif component.grade != grade:
        print(f"{course_code} {student}: attested {component.component} "
          f"result {component.grade} is different from {grade}.")
    except Exception as err:
      try:
        print(f"{course_code} {component_code}={grade} {student}: {err}",
          file=sys.stderr)
      except ValueError as verr:
        print(f"{verr}: "
          f"{course_code} {component_code}={grade} {student_id}: {err}",
          file=sys.stderr)
