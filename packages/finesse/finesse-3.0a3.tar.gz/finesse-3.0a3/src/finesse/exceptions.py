    ...
    def __init__(self, model, pieces):
        ...
        if self.path.endswith("."):
            ...
            if key not in spell.candidates(key):
                msg += (
                    f"\n\nDid you mean: {', '.join([correct + _ for _ in candidates])}?"
                )
        super().__init__(msg)
...
class ModelParameterDefaultValueError(FinesseException):
    ...
    def __init__(self, element):
        super().__init__(
            f"{element.name} cannot be referenced because type "
            f"'{element.__class__.__name__}' has no default model parameter"
        )
