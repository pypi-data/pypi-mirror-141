...
def MAKE_simplify_add(dir):
    def simplify_add(self, other):
        if np.all(self == 0):
            ...
            if dir == "LOP":
                return Function("+", operator_add, *a, *b)
            else:
                return Function("+", operator_add, *b, *a)
...
def MAKE_simplify_pow():
    def simplify_pow(self, exp):
        if np.all(exp == 0):
            ...
            return self
        elif type(self) is Constant and type(exp) is Constant:
...
FUNCTIONS = {
    ...
    "arange": lambda a, b, c: Function(
        "arange", np.arange, float(a), float(b), int(c)
    ),
...
def finesse2sympy(expr, iter_num=0):
    ...
    if isinstance(expr, Constant):
        ...
        if expr.op == operator_mul:
            op = sympy.Mul
        elif expr.op == operator_add:
            op = sympy.Add
...
class Function(Symbol):
    ...
    *args
    ...
    def __hash__(self):
        return hash((self.op, *(hash(_) for _ in self.args)))
