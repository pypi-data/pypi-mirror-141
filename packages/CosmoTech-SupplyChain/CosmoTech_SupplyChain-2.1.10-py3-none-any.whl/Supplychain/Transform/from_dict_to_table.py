...
class FromDictToTableConverter(Timer, ADXAndFileWriter):
    ...
    def convert_file(self,
        ...
        if fixed_data:
            self.write_target_file(fixed_data, fixed_data_filename)
        else:
            ...
            self.write_target_file([fake_fixed_element, ], fixed_data_filename)
        ...
        if temporal_data:
            self.write_target_file(temporal_data, temporal_data_filename)
            ...
            self.write_target_file([fake_temporal_element, ], temporal_data_filename)
        for event_name, event_file_content in event_data.items():
            if event_file_content:
                self.write_target_file(event_file_content, event_name)
                ...
                self.write_target_file([fake_event_element, ], event_name)
