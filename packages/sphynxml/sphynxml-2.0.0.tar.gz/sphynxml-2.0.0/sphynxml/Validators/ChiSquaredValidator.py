...
from SphynxML.utils._validators import _get_counts
from SphynxML.Validators.BaseValidator import BaseUnivariateDrift


class ChiSquaredValidator(BaseUnivariateDrift):
    def __init__(
        self,
        ...
        if isinstance(categories_per_feature, dict):
            ...
            if all(isinstance(v, int_types) for v in vals):
                ...
                categories_per_feature = {
                    f: list(np.arange(v))  # type: ignore
            ...
            categories_per_feature = {
                f: list(np.unique(x_flat[:, f]))  # type: ignore
    ...
    def feature_score(
        ...
        x_ref_count = _get_counts(x_ref, all_categories)
        x_count = _get_counts(x, all_categories)
    ...
    # def _get_counts(
    #     self, x: np.ndarray, categories: Dict[int, List[int]]
    # ) -> Dict[int, List[int]]:
    #     """
    #     Utility method for getting the counts of categories for each categorical variable.
    #     """
    #     return {
    #         f: [(x[:, f] == v).sum() for v in vals] for f, vals in categories.items()
    #     }
