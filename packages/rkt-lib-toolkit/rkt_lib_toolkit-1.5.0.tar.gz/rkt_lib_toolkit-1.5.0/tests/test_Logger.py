...
from rkt_lib_toolkit.tool import Singleton
...
class TestLogger(TestCase):
    ...
    def test_logger_when_log_is_not_directory(self):
        if os.path.exists("tests/log"):
            if os.path.isdir("tests/log"):
                shutil.rmtree("tests/log")
            else:
                os.remove("tests/log")
        open("tests/log", "x").close()
        Singleton._instances = {}
        with self.assertRaises(LogIsNotDirError) as context:
            logger = Logger("coverage", "tests/log")
    ...
    def test_set_logger(self):
        ...
        excepted = f"{logger._tool.get_dir(folder='tests/log')}coverage_{datetime.today().date()}.log"
