...
def str_dict(dicts, str1='', per=None):
    """
    把字典格式的变量格式化为字符串
    :param dicts:
    :param str1:
    ...
    if type(dicts) == dict:
        ...
        str_item = f'{len(dicts)} item in the {type(dicts)}:' if per is None else per
        for key in dicts.keys():
            str_item += f'\n{str1}{key}:'
            str_item += str_dict(dicts[key], str1 + "  ", per=per) + ','
        ...
        str_item = f'{len(dicts)} item in the {type(dicts)}:' if per is None else per
        for item in dicts:
            str_item += f'\n  {str1}{str_dict(item, str1 + "  ", per=per)},'
        return f'[{str_item}\n{str1}]'
    else:
        return f'{dicts}'
