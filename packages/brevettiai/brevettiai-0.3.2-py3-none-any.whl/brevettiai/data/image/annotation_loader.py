...
class AnnotationLoader(FileLoader):
    ...
    def load(self, path, shape=(224, 224), postprocess=True):
        data, meta = super().load(path)
        label_space = self.label_space
        if postprocess and self.postprocessor is not None:
            scale = 1 / np.array(self.postprocessor.scale(shape[0], shape[1])[::-1])
        ...
        annotation = tf.numpy_function(_parse_annotation_buffer, [data, (*shape, len(self.classes))],
