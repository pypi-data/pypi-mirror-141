    ...
    if config_file.exists() and config_file.is_file():
        try:
            ...
            # ENV files
                ...
                logging.warning(f'Configuration file "{config_file}" is of unsupported type')
        except (
            ...
            logging.error(f'Configuration file "{config_file}" cannot be read ({e})')
...
class UserInput(TaskParameter):
    ...
    def configure_value(self, value: Any = None):
        logging.info(f'Asking for user input (Parameter: {self._key})')
        sleep(0.1)
        if not self._value:
...
class UserSelection(TaskParameter):
    ...
    def configure_value(self, value: Any = None):
        logging.info(f'Asking for user selection (Parameter: {self._key})')
        sleep(0.1)
        if not self._value:
            ...
            if (value := convert_parameter(selected)) in self._options:
                ...
                logging.error(f'Selected wrong option "{selected}" for parameter ({self._key})')
