...
class TaskIO:
    ...
    def serialize(self, cache: Path, value: Any = None):
        ...
        try:
            ...
            logging.warning(f'Could not cache value of "{self.task.name}.{self.id}" (Unsupported variable value)')
        finally:
            if cache_file.exists() and cache_file.stat().st_size == 0:
                logging.warning(f'Could not cache value of "{self.task.name}.{self.id}" (Cache file is empty)')
    ...
    def deserialize(self, cache: Path):
        ...
        try:
            ...
            if self.value is None:
                logging.warning(f'Could not read cached value of "{self.task.name}.{self.id}" (Unsupported variable value)')
