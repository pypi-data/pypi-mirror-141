...
class ModelTaskLogger:
    """
    A simple wrapper class wrapping certain common logger methods
    and adding the task name as prefix.
    """

    logger: None

    def __init__(self, logger: logging.Logger = None, task: str = None):
        self.logger = logger or logging.getLogger()
        self.task = task or ''

    def __getattr__(self, name):
        try:
            attribute = self.logger.__getattribute__(name)
            if callable(attribute):
                return self.__wrap_logger_method(name, attribute)
            else:
                return attribute
    ...
    def __wrap_logger_method(self, name, method):
        @wraps(method)
        def wrapper(*args, **kwargs):
            if name in ('info', 'warning', 'debug', 'error', 'critical', 'log', 'fatal'):
                args = [f'Task "{self.task}": {args[0]}']
...
class ModelTask(metaclass=ModelTaskMount):
    ...
    def __init__(self, task_id: str = None):
        ...
        logging.debug(f'Initializing model task "{self.name}"')
            ...
            if issubclass(a_type, TaskOutput):
                ...
                try:
                    ...
                    if hasattr(variable, '_get_other_value') and variable.is_set:
                        ...
                        logging.warning(f'Task variable "{attribute}" is already configured with a value (Do not attempt to modify existing value)')
    ...
    def run(self, logger: logging.Logger, workspace: Path):
        logger.info(f'Running model task "{self.name}"')

    def on_finished(self, logger: logging.Logger, workspace: Path):
