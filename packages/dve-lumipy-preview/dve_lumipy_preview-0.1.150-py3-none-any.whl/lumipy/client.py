...
class Client:
    ...
    def _429_handling(self, action: Callable, label: str):
        """Wrap an api call and handle 429 retries and present more readable information from exceptions.
    ...
    def get_result(self, execution_id: str, page_size: Optional[int] = 100000, sort_by: Optional[str] = None, filter_str: Optional[str] = None, verbose: bool = False):
        ...
        qry_state = status['state']
        row_count = int(status['row_count'])

        if row_count == -1:
            raise ValueError(f"Result unavailable: query state = {qry_state}")
