    ...
    try:
        ...
        with h5py.File(output_name, 'w') as f:
            ...
            for m in mixid:
                ...
                indices = slice(sample_offset, sample_offset + samples)
        ...
        logger.info('')
        logger.info(f'Duration: {seconds_to_hms(seconds=duration)}')
        logger.info(f'mixture:  {human_readable_size(sample_offset * 2, 1)}')
        if compute_truth:
            logger.info(f'truth_t:  {human_readable_size(sample_offset * mixdb_out["num_classes"] * 4, 1)}')
        logger.info(f'target:   {human_readable_size(sample_offset * 2, 1)}')
        logger.info(f'noise:    {human_readable_size(sample_offset * 2, 1)}')
        if compute_segsnr:
            logger.info(f'segsnr:   {human_readable_size(sample_offset * 4, 1)}')
