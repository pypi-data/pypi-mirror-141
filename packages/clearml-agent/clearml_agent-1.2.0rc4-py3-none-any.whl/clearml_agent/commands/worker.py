...
class Worker(ServiceCommandSection):
    ...
    def execute(
        ...
        **_
        ...
        custom_build_script = self._session.config.get("agent.custom_build_script", ENV_CUSTOM_BUILD_SCRIPT.get())
        if custom_build_script:
            try:
                ...
                executable_name, entry_point, working_dir, requirements_manager = self.run_custom_build_script(
                    custom_build_script,
                    current_task,
                    venv_folder=venv_folder,
                    git_root=vcs.location,
                )
                executable_version_suffix = self._get_python_version_suffix(str(executable_name))
                ...
                current_task.script.entry_point = entry_point
    ...
    def run_custom_build_script(self, script, task, venv_folder, git_root):
        # type: (str, dict, Path, str)-> Tuple[Path, Path, Path, RequirementsManager]
        """
        ...
        os.environ["CLEARML_CUSTOM_BUILD_TASK_CONFIG_JSON"] = json.dumps(task, separators=(',', ':'))
        os.environ["CLEARML_CUSTOM_BUILD_OUTPUT"] = script_output_file.name
        os.environ["CLEARML_TASK_SCRIPT_ENTRY"] = task.script.entry
        os.environ["CLEARML_TASK_WORKING_DIR"] = task.script.working_dir
        os.environ["CLEARML_VENV_PATH"] = str(venv_folder)
        os.environ["CLEARML_GIT_ROOT"] = git_root

        try:
            subprocess.check_call([script_output_file])
        except subprocess.CalledProcessError as ex:
            print("Custom build script failed with return code {}".format(ex.returncode))
            raise CustomBuildScriptFailed(ex.returncode)
        ...
        try:
            ...
            raise SkippedCustomBuildScript("Build script output missing {}".format(ex.args[0]))

        if not binary.is_file():
            print("Warning: invalid binary path returned from custom build script: {}".format(binary))
        if not entry_point.is_file():
            print("Warning: invalid entrypoint path returned from custom build script: {}".format(entry_point))
        if not working_dir.is_dir():
            print("Warning: invalid working dir returned from custom build script: {}".format(working_dir))

        requirements_manager = self._get_requirements_manager(
            base_interpreter=binary
        )

        return binary, entry_point, working_dir, requirements_manager
