...
_archive_filename = "compressed.tar.gz"
_archive_name = f'{_dir_name}/{_archive_filename}'
_expectation_dir = "_test_directory_compress_to_tar/source"
...
def f(archive_filename, dir_name):
  return run(
    args=f"tar -xvzf {archive_filename}".split(),
    capture_output=True,
    cwd=dir_name
  )
...
def t():
  setup()
  f(_archive_filename, _dir_name)
  passed = all([
    load(f"./{_expectation_dir}/a.txt") == '_source_file_a_path',
    load(f"./{_expectation_dir}/b.txt") == '_source_file_b_path'
  ])
  tear_down()
  return passed
