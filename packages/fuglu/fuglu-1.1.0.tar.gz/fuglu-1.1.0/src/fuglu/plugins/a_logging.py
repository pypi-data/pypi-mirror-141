...
class ElasticBackend(object):
    ...
    def log_to_elastic(self, suspect:Suspect, logdata:tp.Dict, reload=False, retry:int=3) -> tp.Dict[str, str]:
        result = {}
        try:
            ...
            if retry>0:
                ...
                result = self.log_to_elastic(suspect, logdata, reload=True, retry=retry-1)
            else:
                raise
        return result
...
class ElasticLogger(AppenderPlugin):
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars = {
            ...
            'log_headers':{
                'default': 'from:addr,to:pseudo:md5,reply-to:addr,subject,subject:decode,subject:hash:md5,message-id',
                'description:': """
            ...
            'log_values': {
                'default': 'id,from_address,from_domain,from_localpart,queue_id,sasl_login,size,timestamp,to_address,to_domain,to_localpart,to_address:pseudo:md5,to_localpart:pseudo:md5',
                'description:': """
                    Name of suspect attribute values to log and extract. Some values will be renamed.
                    Email address like values support :pseudo:algo (hash localpart)"""
            },
            'log_tags':{
                'default': 'injectqueueid,fuzor.digest,log.scanhost,log.decision,log.client_helo,log.client_ip,log.client_hostname,archived,Attachment.bounce.queueid,spf.result,dkim.result,arc.result,dmarc.result,log.real_from_address',
    ...
    def _log_to_file(self, suspect:Suspect, logdata:tp.Dict) -> None:
