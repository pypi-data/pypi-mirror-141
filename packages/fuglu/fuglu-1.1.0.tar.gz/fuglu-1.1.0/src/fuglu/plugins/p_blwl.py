...
import traceback
import typing as tp
from fuglu.shared import PrependerPlugin, AppenderPlugin, SuspectFilter, get_default_cache, Suspect, apply_template, DUNNO, REJECT, ACCEPT, DEFER
...
class BlockWelcomeEntry(object):
    ...
    def __str__(self):
        return f'<BlockWelcomeEntry type={self.list_type} scope={self.scope} addr={self.sender_addr} host={self.sender_host}/{self.netmask}>'
    ...
    *.fqdn: recursively matching fqdn
    ...
    def _get_listings(self, fugluid: str = "") -> tp.Dict[str, tp.List[tp.Union[BlockWelcomeEntry, BlockWelcomeTable]]]:
        ...
        if usecache:
            ...
            self.logger.debug(f"{fugluid} Get {len(listings)} listings from cache as {listings}")
        if not listings:
            self.logger.debug(f"{fugluid} No cached listings, get")
            if listings is None:
                listings = {}
            for func in [self._get_sql, self._get_rest]:
                self.logger.debug(f"{fugluid} No cached listings, get using function {func}")
                items = func(fugluid)
    ...
    def _check_listings(self, check_listings, sender_address, sender_hostname, sender_hostip, listscope, fugluid: str = ""):
        ...
        for listing in check_listings:
            print(f"Check listing: "
                  f"list_scope:{listing.list_scope}, "
                  f"sender_addr:{listing.sender_addr}, "
                  f"sender_host:{listing.sender_host}, "
                  f"sender_netmask:{listing.netmask}")
            if not listing.list_scope in (BlockWelcomeEntry.SCOPE_ANY, listscope):
                self.logger.debug(f"{fugluid} checking listing: {listing} -> skip due to not in scope")
                continue
    ...
    def _check_user_listings(self, suspect, all_listings):
        ...
        self.logger.debug(f"{suspect.id} Check listings returned: {listing}")
        if not listing:
    ...
    def evaluate(self, suspect:Suspect):
        if not SQL_EXTENSION_ENABLED and not self.config.get(self.section, 'fublwl_restapi_uri'):
            self.logger.debug(f"{suspect.id} Skip because SQL_EXTENSION is not enabled and fublwl_restapi is false")
            return
        ...
        try:
            ...
            suspect.set_tag('restapi.error', True)
            listings = {}
        listing, list_scope = self._check_user_listings(suspect, listings)
        self.logger.debug(f"{suspect.id} after applying user -> listing: {listing}, list_scpoe: {list_scope}")
    ...
    def evaluate(self, suspect:Suspect):
        ...
        for option in welcome_options:
            try:
                ...
                errors.add(str(e))
                self.logger.debug('%s %s %s' % (suspect.id, e.__class__.__name__, str(e)))
            except RESTAPIError:
                suspect.set_tag('restapi.error', True)
        ...
        for option in value_options:
            try:
                ...
                errors.add(str(e))
                self.logger.info('%s %s %s' % (suspect.id, e.__class__.__name__, str(e)))
            except RESTAPIError:
                suspect.set_tag('restapi.error', True)
