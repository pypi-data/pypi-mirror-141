...
class URIExtract(ScannerPlugin):
    ...
    def _normalise_components(self, uri):
        ...
        try:
            ...
            netloc = re.sub('\s', '', parsed_uri.netloc)
            netloc = urlparse.unquote(netloc).encode('idna').decode()
    ...
    def _quickfilter(self, uris):
        ...
        uris = [self._normalise_components(self._removeprefixes(u.strip().lstrip('*'), ['blob:', ': ', '3D"', '=3D"'])) for u in uris]
