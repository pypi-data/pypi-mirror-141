...
def correctauto1(
    ...
    for e in sunder.getexps(
        inst, experiments, experimentincludes, experimentexcludes
    ):
        for c in sunder.getcons(
            ...
            refstrfn = processref1(inst, f, refstrain, figs, e, c)
            # correct strains
            for s in sunder.getstrs(
                inst, strains, strainincludes, strainexcludes, nonull=True
            ):
                if not inst.sc[
                    (inst.sc.experiment == e)
                    & (inst.sc.condition == c)
                    & (inst.sc.strain == s)
                ][f[0] + " corrected for autofluorescence"].any():
                    od, rawfl = sunder.extractwells(
                        inst.r, inst.s, e, c, s, ["OD", f[0]]
                    )
                    # no data
                    ...
                    if bname not in inst.s.columns:
                        # extend dataframe
                        inst.s = pd.merge(inst.s, autofdf, how="outer")
                    else:
                        # update dataframe
                        inst.s = gu.absorbdf(
                            inst.s,
...
def processref1(inst, f, refstrain, figs, experiment, condition):
    """
    ...
    od, fl = sunder.extractwells(inst.r, inst.s, e, c, refstrain, ["OD", f[0]])
...
def correctauto2(
    ...
    for e in sunder.getexps(
        ...
        for c in sunder.getcons(
            ...
            # process reference strain
            refqrfn = processref2(inst, f, refstrain, figs, e, c)
            ...
            for s in sunder.getstrs(
                ...
                if s != refstrain and not (
                    ...
                    f0, f1 = sunder.extractwells(inst.r, inst.s, e, c, s, f)
                    ...
                    ra = refqrfn(odmean)
                    fl = applyautoflcorrection(inst, ra, f0, f1)
                    od = sunder.extractwells(inst.r, inst.s, e, c, s, "OD")
                    flperod = fl / od
                    # set negative values to NaNs
                    fl[fl < 0] = np.nan
                    flperod[flperod < 0] = np.nan
...
def processref2(inst, f, refstrain, figs, experiment, condition):
    """
    ...
    f0, f1, od = sunder.extractwells(
        inst.r, inst.s, e, c, refstrain, f + ["OD"]
    )
    if f0.size == 0 or f1.size == 0 or od.size == 0:
        ...
        def choosefrac(frac):
            ...
            flref = applyautoflcorrection(inst, qrfn(odrefmean), f0, f1)
        ...
        if bname not in inst.s.columns:
            inst.s = pd.merge(inst.s, pd.DataFrame(autofdict), how="outer")
        else:
            inst.s = gu.absorbdf(
                inst.s,
                pd.DataFrame(autofdict),
...
def applyautoflcorrection(inst, ra, f0data, f1data):
    """
    ...
    return (raa * f0data - f1data) / (
        raa - inst.gamma * np.ones(np.shape(raa))
    )
