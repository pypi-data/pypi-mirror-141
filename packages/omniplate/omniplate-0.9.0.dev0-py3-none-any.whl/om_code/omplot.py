...
def plotfinddf(inst, x, y):
    ...
    if hasattr(inst, "r") and x in inst.r.columns and y in inst.r.columns:
        # raw data (with wells)
        basedf = inst.r
        dfname = "r"
    elif x in inst.s.columns and y in inst.s.columns:
        # processed data (no wells)
        basedf = inst.s
        dfname = "s"
    elif x in inst.sc.columns and y in inst.sc.columns:
        # summary stats
        basedf = inst.sc
        dfname = "sc"
    else:
