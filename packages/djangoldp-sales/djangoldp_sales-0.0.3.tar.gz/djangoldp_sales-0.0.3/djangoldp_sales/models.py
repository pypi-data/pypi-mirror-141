...
class Order(Model):
    ...
    sale = models.ForeignKey(Sale, blank=True, null=True, related_name="saleorder", max_length=50, on_delete=models.CASCADE, verbose_name="Vente")
    product = models.ForeignKey(Product, blank=True, max_length=50, on_delete=models.CASCADE, verbose_name="produits", related_name="productorder")
    orderquantity =  models.IntegerField(blank=True, null=True, verbose_name="Quantité commandée")

    def __str__(self):
        return "%s - %s - %s" % (self.client.username , self.sale , self.product)
...
class Productstock(Model):
    ...
    order = models.ForeignKey(Order, blank=True, max_length=50, on_delete=models.CASCADE, verbose_name="commande", related_name="orderstock")
...
def create_order(sender, instance, created, **kwargs):
    if created:
        print("Nouvelle commande pouet pouet!")
        stockInstance = Productstock(
            product = instance.product,
            stockRemove = instance.orderquantity,
            order = instance
        )
        stockInstance.save()
