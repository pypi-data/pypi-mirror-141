...
def regler_biais(appareil, v: float, delai=0.01) -> HP4274AState:
    'Commande pour régler le potentiel de biais'
...
def regler_freq(appareil, f: float, delai=0.01) -> HP4274AState:
    'Commande pour régler la fréquence du signal AC'
    i: int = fs.index(f)
...
def exe(root,
    ...
    vb: list[float] = list(linspace(min_vb, max_vb, nbr))  # En potentiel de biais
    ...
    sauvegarde /= datetime.datetime.now().isoformat().replace(':', '_').replace('.', '_')
    sauvegarde.mkdir()
...
{nom_ressource=}'''
    ...
    try:
        ...
        for f in freqs:
            ...
            for v in vb:
                ...
                if res.data_status_A == 'N' or res.value_A < 0:
                    ...
                    ax.set_yticks(cs[::int(len(cs)/10)], [f'{c:.2e}' for c in cs[::int(len(cs)/10)]])
        ...
        mb.showerror(msg := f'{type(e)}', f'Une erreur {e!r} est survenue, voir console ou journal.')
...
def main():
    'Interface graphique pour prise de mesures'
    ...
    combo_adresse = ttk.Combobox(
        root, values=ressources, textvariable=ressource_var, exportselection=True)
    ...
    max_vb_entry = ttk.Spinbox(root, from_=-35, to=35, increment=0.1, textvariable=max_vb_var)
    min_vb_entry = ttk.Spinbox(root, from_=-35, to=35, increment=0.1, textvariable=min_vb_var)
    ...
    max_freq_entry = ttk.Spinbox(root, from_=100, to=1e5, increment=100, textvariable=max_freq_var)
    min_freq_entry = ttk.Spinbox(root, from_=100, to=1e5, increment=100, textvariable=min_freq_var)
    ...
    matricule_entry.grid(row=1, column=1, sticky=tk.E+tk.W, padx=pad, pady=pad)
    composante_label.grid(row=2, column=0, sticky=tk.E, padx=pad, pady=pad)
    composante_entry.grid(row=2, column=1, sticky=tk.E+tk.W, padx=pad, pady=pad)
    ...
    min_freq_entry.grid(row=5, column=1, sticky=tk.E+tk.W, padx=pad, pady=pad)
    max_freq_label.grid(row=6, column=0, sticky=tk.E, padx=pad, pady=pad)
    max_freq_entry.grid(row=6, column=1, sticky=tk.E+tk.W, padx=pad, pady=pad)
    ...
    progres.grid(row=14, column=0, columnspan=2, sticky=tk.E+tk.W, padx=pad, pady=pad)
    canvas.get_tk_widget().grid(row=0, column=2, rowspan=14, padx=pad, pady=pad)
