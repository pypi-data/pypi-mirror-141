...
class Client:
    ...
    def get_inputs_from_feed(
        ...
        where = [{"key": k, "value": str(v)} for k, v in where.items()]
        ...
        query = """
        query EventFeed($from: Int!, $size: Int!, $end: DateTime!, $eventTypes: [EventSelection], $where: [FeatureFilter]!) {
            events {
                feed(
                    from: $from
                    size: $size
    ...
    def get_features_from_feed(
        ...
        query = """
        query EventFeed($from: Int!, $size: Int!, $end: DateTime!, $eventType: String!, $where: [FeatureFilter]!) {
            events {
                feed(
                    from: $from
                    size: $size
