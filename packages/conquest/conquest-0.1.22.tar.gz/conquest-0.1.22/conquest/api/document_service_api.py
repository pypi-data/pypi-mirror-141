    ...
    def document_service_add_document_with_http_info(self, body, **kwargs):  # noqa: E501
        ...
        if filename:
            ...
            with open(filename, 'rb', encoding=enc) as f:
                data = f.read()
            body.content_type = body.content_type if body.content_type else mtype
        if body.address:
            body.address = datetime.now().strftime('%Y%m%d%H%M%S%d') + str(body.address)
            mtype, enc = mimetypes.guess_type(body.address)
            body.content_type = body.content_type if body.content_type else mtype
