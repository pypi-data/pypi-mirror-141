...
class QLDB():
    ...
    def __init__(self, table, ledger=settings.LEDGER):
        """Creates an instance of `qldb_orm.qldb.QLDB`. This class representation some basic configuration properties of the **QLDB** ledger.

        :param table: Name of the table onto which transactions will read and write.
        :type table: str
        :param ledger: Name of the ledger where tables are stored, defaults to `qldb_orm.settings.LEDGER`
...
class Document(QLDB):
    ...
    :param ledger: Name of the **QLDB** ledger, defaults to `qldb_orm.settings.LEDGER`
    :type ledger: str, optional
    ...
    def _init_history(self):
        """Initializes the `qldb_orm.qldb.Document` revision history. After this method is invoked, the `self.strands` attribute will be populated with an array of `qldb_orm.qldb.Document` ordered over the revision history from earliest to latest.
        """
        self.strands = []
    ...
    def _load(self, snapshot=None, nest=None, nester=None):
        """Parse the `snapshot` into `qldb_orm.qldb.Document` attributes. If `nest` and `nester` are passed in, the function executes recursively, drilling down through the nodes in the `snapshot` and recursively generating the document structure.
    ...
    def _exists(self, id, snapshot=False):
        """Check if a `qldb_orm.qldb.Document` exists in the **QLDB** ledger table.

        :param id: ID of the `qldb_orm.qldb.Document` whose existence is in question.
        :type id: str
    ...
    def fields(self):
        ...
        :return: `qldb_orm.qldb.Document` fields
        :rtype: dict
        """
...
class Query(QLDB):
    ...
    def _to_documents(self, results):
        """Convert query results to a `list` of `qldb_orm.qldb.Document`
    ...
    def raw(self, query):
        ...
        :type query: str
        :return: a collection of `qldb_orm.qldb.Document`
        :rtype: list
    ...
    def get_all(self):
        ...
        :return: List of `qldb_orm.qldb.Document`
        :rtype: list
        """
