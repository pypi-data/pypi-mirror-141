...
class SilaServiceListener(ServiceListener):
    ...
    def __add_client(self, service_info: Optional[ServiceInfo] = None) -> None:
        ...
        try:
            ...
            if ca is not None:
                self.services[service_name] = SilaClient(ip_address, port, root_certs=ca)
                warnings.warn(
                    RuntimeWarning(
                        f"Connected to server at {ip_address}:{port} with UUID {service_uuid} via untrusted certificate"
                    )
                )
            elif self.cert_chain is not None or self.private_key is not None or self.root_certs is not None:
                ...
                self.services[service_name] = SilaClient(ip_address, port, insecure=True)
            else:
                warnings.warn(
                    RuntimeWarning(f"Ignored unencrypted server at {ip_address}:{port} with UUID {service_uuid}")
                )
