...
class Report(object):
    ...
    def __init__(self, creturn, position, fee_ratio, tax_ratio, trade_at, next_trading_date):
        ...
        self.asset_type = 'tw_stock' if (position.columns.str.find('USDT') == -1).all() else 'crypto'
    ...
    def get_price(self, price_at, adj=True):
        ...
        if self.asset_type == 'tw_stock':
          ...
          if adj:
            ...
            chname = {'open':'開盤價', 'close':'收盤價', 'high':'最高價', 'low':'最低價', 'volume':'成交股數'}
    ...
    def upload(self, name=None, mae_mfe_charts=False):
        ...
        if self.asset_type == 'crypto':
          return_ = self.creturn.resample('1d').last().ffill().dropna().rebase()
        else:
        ...
        mae_mfe_plotly_html = to_html(fig, full_html=False, include_plotlyjs='cdn') if fig else None
    ...
    def position_info(self, position):
        ...
        present_date = position_change_dates[-1] if len(position_change_dates) > 0 else position.index[0]
        previous_date = position_change_dates[-2] if len(position_change_dates) > 1 else position.index[0]
    ...
    def get_trade_record(self, time_scale=None, edge_ratio_series=False, edge_ratio_time_range=None, atr_freq=21):
        ...
        for stock_id in sig.columns:
            ...
            for entry, exit_, pos in zip(entry_sig_date, exit_sig_date, target_positions):
                trade_record = {'stock_id': stock_id, 'entry_sig_date': entry, 'exit_sig_date': exit_, 'position': pos}
                record_set.append(trade_record)

        df = pd.DataFrame(record_set)
        target_price = data.get(f'etl:adj_{self.trade_at}').ffill().shift(-1).dropna(how='all')
        ...
        def get_mae_mfe_price(stock_id, entry_sig_date, exit_sig_date, position=None, time_scale=None):
            result = {}
            if time_scale:
                price_period = target_price.loc[entry_sig_date:, stock_id].iloc[:time_scale]
                atr = atr_df.loc[entry_sig_date:, stock_id].iloc[:time_scale][-1]
            else:
                ...
                mdd = abs(round(((price_period / price_period.cummax() - 1) * 100).min(), 2))
                result['mdd'] = mdd
        ...
        def get_fluctuation(time_scale=None):
            ...
            for return_name, exit_col in [('return', 'exit_price'), ('mae_return', 'mae_price'),
                ...
                if return_name in ['mae_return']:
                    ...
                    fluctuation[return_name] = fluctuation[return_name].apply(lambda s: 0 if s < 0 else s)
                fluctuation[return_name] = fluctuation[return_name].apply(
                    lambda s: 0 if (np.isinf(s)) or (pd.isna(s)) else s)
            fluctuation['atr_return'] = round(fluctuation['atr'] / fluctuation['entry_price'] * 100, 2)
            fluctuation['edge_ratio'] = fluctuation['g_mfe_return'] / fluctuation['mae_return']
            return fluctuation
        ...
        if edge_ratio_series:
            ...
            for t in edge_ratio_time_range:
                ...
                normal_g_mfe_return = fluctuation['g_mfe_return'] / fluctuation['atr_return']
                normal_g_mfe_return = normal_g_mfe_return[normal_g_mfe_return != np.inf]
                normal_mae_return = fluctuation['mae_return'] / fluctuation['atr_return']
                ...
                edge_ratio_dataset.append({'time_scale': t, 'mean_edge_ratio': mean_value, 'mean_atr': mean_atr})

        # get entry_date & exit_date
        date_map = pd.DataFrame({'date_map': target_price.index}, index=target_price.index).shift(-1)
    ...
    def display_mae_mfe_analysis(self, plot_data=None, stop_loss=0, take_profit=0, edge_ratio=False,
        ...
        try:
            ...
            if edge_ratio_series.empty is False:
                fig_edge_ratio_hist = px.bar(edge_ratio_series, x='time_scale', y='mean_edge_ratio')
                fig_atr_line = px.line(edge_ratio_series, x='time_scale', y='mean_atr')
...
def cal_atr(time_period=14):
    ...
    tr_df = pd.concat([tr_cond1, tr_cond2, tr_cond3], axis=1).max(axis=1).unstack(level=0)
