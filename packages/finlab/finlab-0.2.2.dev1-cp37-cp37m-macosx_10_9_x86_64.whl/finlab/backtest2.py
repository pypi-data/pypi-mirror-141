...
def arguments(price, position, resample_dates=None):
  ...
  if isinstance(resample_dates, list):
    resample_dates = pd.Series(resample_dates).astype(int).values
  elif isinstance(resample_dates, pd.Series):
    resample_dates = resample_dates.astype(int).values
  elif isinstance(resample_dates, pd.DatetimeIndex):
    resample_dates = resample_dates.astype(int).values
  else:
    raise Exception('Resample dates must be numpy array')

  position = position.astype(float)
  price = price.astype(float)

  return [price.values, price.index.astype(int).values, price.columns.astype(str).values,
          position.values, position.index.astype(int).values, position.columns.astype(str).values,
          resample_dates
         ]
...
def sim(position:pd.DataFrame, resample=None, trade_at_price='close',
    ...
    d = pd.DataFrame(
      get_trade_stocks(position.columns.astype(str).values, price.index.astype(int).values))
    ...
    if len(d) != 0:
      ...
      tuples = sum([[(n*mae_mfe_window_step, metric) for metric in ['mae', 'g_mfe', 'b_mfe', 'mdd']] for n in list(range(nsets))], [])\
        +[('exit', 'mae'),('exit', 'gmfe'),('exit', 'bmfe'),('exit', 'mdd')]+ [('exit', 'return')]
    ...
    report_ret = report.Report(creturn, position, fee_ratio, tax_ratio, trade_at_price, next_trading_date)
    if len(d)!=0:
      report_ret.trades = d
