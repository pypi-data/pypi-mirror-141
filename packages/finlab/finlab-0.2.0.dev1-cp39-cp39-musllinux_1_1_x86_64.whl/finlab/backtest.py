...
def arguments(price, position, resample_dates=None):
    ...
    if isinstance(resample_dates, list):
        resample_dates = pd.Series(resample_dates).view(int).values
    elif isinstance(resample_dates, pd.Series):
        resample_dates = resample_dates.view(int).values
    ...
    return [price.values,
            price.index.view(int),
            price.columns.astype(str).values,
            position.values,
            position.index.view(int),
...
def sim(position: pd.DataFrame, resample=None, trade_at_price='close',
    ...
    d = pd.DataFrame(
        get_trade_stocks(position.columns.astype(str).values, price.index.view(int)))
