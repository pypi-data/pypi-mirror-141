...
def connect_with_backoff(connection):
    ...
    with connection.cursor() as cur:
        ...
        try:
            cur.execute('SET @@session.wait_timeout=28800') # custom
        except pymysql.err.InternalError as e:
        ...
        try:
            cur.execute('SET @@session.innodb_lock_wait_timeout=3600') #custom
        except pymysql.err.InternalError as e:
            warnings.append(
        ...
        try:
            cur.execute('SET @@session.max_execution_time=0') # custom
        except pymysql.err.InternalError as e:
            warnings.append('Could not set session.max_execution_time. Error: ({}) {}'.format(*e.args))
