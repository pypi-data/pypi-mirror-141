...
class Dbt:
    ...
    def run(self, select: str = "", exclude: str = "", refresh: bool = False, target: str = "dev"):
        ...
        Args:
            ...
            target (:obj:`str`, optional): set dbt profile target
        """
        self.transpile()
        cmd = ["dbt", "run", "--fail-fast", "--target", target]
        if refresh:
