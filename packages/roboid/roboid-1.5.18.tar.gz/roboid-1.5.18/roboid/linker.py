...
class Linker(object):
    ...
    @staticmethod
    def _get_or_create_link(group, module, index, tag, roboid):
        index = str(index)
        key = module + index
        if key in Linker._links:
            link = Linker._links[key]
        else:
            link = Link(tag, roboid)
    ...
    def register_roboid(group, module, index, tag, roboid):
        Linker._get_or_create_link(group, module, index, tag, roboid)
