...
class TurtleLinkRoboid(TurtleRoboid):
    def __init__(self, index):
        super(TurtleLinkRoboid, self).__init__(index)
        self._motoring = {
            'module': 'turtle',
            'index': index
        }
    ...
    def _init(self, loader, port_name=None):
        self._tag = "Turtle[{}]".format(self._index)
        Linker.register_roboid('turtle', 'turtle', self._index, self._tag, self)
        while self._ready == False and self._is_disposed() == False:
            time.sleep(0.01)

    def _release(self):
        super(TurtleLinkRoboid, self)._release()
    ...
    def _clear_id_and_motoring(self):
        ...
        motoring['ledRed'] = self._led_red
        motoring['ledGreen'] = self._led_green
        motoring['ledBlue'] = self._led_blue
        ...
        motoring['lampId'] = 0
        motoring['lamp'] = self._lamp
        motoring['lockId'] = 0
        motoring['lock'] = self._lock

    def _reset(self):
        super(TurtleLinkRoboid, self)._reset()
        self._clear_id_and_motoring()
    ...
    def encode_motoring(self):
        with self._thread_lock:
            ...
            if self._lamp_written:
                self._lamp_written = False
                motoring['lamp'] = self._lamp
                self._lamp_id = (self._lamp_id % 255) + 1
                motoring['lampId'] = self._lamp_id
            if self._lock_written:
                self._lock_written = False
                motoring['lock'] = self._lock
                self._lock_id = (self._lock_id % 255) + 1
                motoring['lockId'] = self._lock_id
    ...
    def decode_sensory(self, received):
        ...
        self._color_device._put_at(3, received['colorClear'])
