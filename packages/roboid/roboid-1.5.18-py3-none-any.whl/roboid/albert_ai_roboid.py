...
class AlbertAiLinkRoboid(AlbertAiRoboid):
    def __init__(self, index):
        super(AlbertAiLinkRoboid, self).__init__(index)
        self._motoring = {
            'module': 'albertai',
            'index': index
        }
    ...
    def _init(self, port_name=None):
        self._tag = "AlbertAi[{}]".format(self._index)
        Linker.register_roboid('albertai', 'albertai', self._index, self._tag, self)
        while self._ready == False and self._is_disposed() == False:
            time.sleep(0.01)

    def _release(self):
        super(AlbertAiLinkRoboid, self)._release()
    ...
    def _clear_id_and_motoring(self):
        ...
        self._note_id = 0
        self._sound_id = 0
        self._board_size_id = 0
        self._motion_id = 0
        self._mic_touch_id = -1
        self._volume_up_touch_id = -1
        self._volume_down_touch_id = -1
        self._play_touch_id = -1
        self._back_touch_id = -1
        self._mic_clicked_id = -1
        self._volume_up_clicked_id = -1
        self._volume_down_clicked_id = -1
        self._play_clicked_id = -1
        self._back_clicked_id = -1
        self._mic_long_pressed_id = -1
        self._volume_up_long_pressed_id = -1
        self._volume_down_long_pressed_id = -1
        self._play_long_pressed_id = -1
        self._back_long_pressed_id = -1
        self._mic_long_long_pressed_id = -1
        self._volume_up_long_long_pressed_id = -1
        self._volume_down_long_long_pressed_id = -1
        self._play_long_long_pressed_id = -1
        self._back_long_long_pressed_id = -1
        self._tap_id = -1
        self._oid_mode_id = -1
        self._oid_id = -1
        self._lift_id = -1
        ...
        motoring['leftRed'] = self._left_eye_red
        motoring['leftGreen'] = self._left_eye_green
        motoring['leftBlue'] = self._left_eye_blue
        motoring['rightRed'] = self._right_eye_red
        motoring['rightGreen'] = self._right_eye_green
        motoring['rightBlue'] = self._right_eye_blue
        motoring['micRed'] = self._mic_led_red
        motoring['micGreen'] = self._mic_led_green
        motoring['micBlue'] = self._mic_led_blue
        ...
        motoring['boardSizeId'] = 0
        motoring['boardWidth'] = self._board_width
        motoring['boardHeight'] = self._board_height

    def _reset(self):
        super(AlbertAiLinkRoboid, self)._reset()
        self._clear_id_and_motoring()
    ...
    def encode_motoring(self):
        with self._thread_lock:
            ...
            if self._board_size_written:
                self._board_size_written = False
                motoring['boardWidth'] = self._board_width
                motoring['boardHeight'] = self._board_height
                self._board_size_id = (self._board_size_id % 255) + 1
                motoring['boardSizeId'] = self._board_size_id
    ...
    def decode_sensory(self, received):
        ...
        self._position_device._put_at(0, received['positionX'])
        self._position_device._put_at(1, received['positionY'])
        self._orientation_device._put(received['orientation'])
        self._light_device._put(received['light'])
        self._battery_device._put(received['battery'])
        t = received['micTouchId']
        if t != self._mic_touch_id:
            if self._mic_touch_id != -1: self._mic_touch_device._put(received['micTouch'])
            self._mic_touch_id = t
        t = received['volumeUpTouchId']
        if t != self._volume_up_touch_id:
            if self._volume_up_touch_id != -1: self._volume_up_touch_device._put(received['volumeUpTouch'])
            self._volume_up_touch_id = t
        t = received['volumeDownTouchId']
        if t != self._volume_down_touch_id:
            if self._volume_down_touch_id != -1: self._volume_down_touch_device._put(received['volumeDownTouch'])
            self._volume_down_touch_id = t
        t = received['playTouchId']
        if t != self._play_touch_id:
            if self._play_touch_id != -1: self._play_touch_device._put(received['playTouch'])
            self._play_touch_id = t
        t = received['backTouchId']
        if t != self._back_touch_id:
            if self._back_touch_id != -1: self._back_touch_device._put(received['backTouch'])
            self._back_touch_id = t
        t = received['micClickedId']
        if t != self._mic_clicked_id:
            if self._mic_clicked_id != -1: self._mic_clicked_device._put_empty()
            self._mic_clicked_id = t
        t = received['volumeUpClickedId']
        if t != self._volume_up_clicked_id:
            if self._volume_up_clicked_id != -1: self._volume_up_clicked_device._put_empty()
            self._volume_up_clicked_id = t
        t = received['volumeDownClickedId']
        if t != self._volume_down_clicked_id:
            if self._volume_down_clicked_id != -1: self._volume_down_clicked_device._put_empty()
            self._volume_down_clicked_id = t
        t = received['playClickedId']
        if t != self._play_clicked_id:
            if self._play_clicked_id != -1: self._play_clicked_device._put_empty()
            self._play_clicked_id = t
        t = received['backClickedId']
        if t != self._back_clicked_id:
            if self._back_clicked_id != -1: self._back_clicked_device._put_empty()
            self._back_clicked_id = t
        t = received['micLongPressedId']
        if t != self._mic_long_pressed_id:
            if self._mic_long_pressed_id != -1: self._mic_long_pressed_device._put_empty()
            self._mic_long_pressed_id = t
        t = received['volumeUpLongPressedId']
        if t != self._volume_up_long_pressed_id:
            if self._volume_up_long_pressed_id != -1: self._volume_up_long_pressed_device._put_empty()
            self._volume_up_long_pressed_id = t
        t = received['volumeDownLongPressedId']
        if t != self._volume_down_long_pressed_id:
            if self._volume_down_long_pressed_id != -1: self._volume_down_long_pressed_device._put_empty()
            self._volume_down_long_pressed_id = t
        t = received['playLongPressedId']
        if t != self._play_long_pressed_id:
            if self._play_long_pressed_id != -1: self._play_long_pressed_device._put_empty()
            self._play_long_pressed_id = t
        t = received['backLongPressedId']
        if t != self._back_long_pressed_id:
            if self._back_long_pressed_id != -1: self._back_long_pressed_device._put_empty()
            self._back_long_pressed_id = t
        t = received['micLongLongPressedId']
        if t != self._mic_long_long_pressed_id:
            if self._mic_long_long_pressed_id != -1: self._mic_long_long_pressed_device._put_empty()
            self._mic_long_long_pressed_id = t
        t = received['volumeUpLongLongPressedId']
        if t != self._volume_up_long_long_pressed_id:
            if self._volume_up_long_long_pressed_id != -1: self._volume_up_long_long_pressed_device._put_empty()
            self._volume_up_long_long_pressed_id = t
        t = received['volumeDownLongLongPressedId']
        if t != self._volume_down_long_long_pressed_id:
            if self._volume_down_long_long_pressed_id != -1: self._volume_down_long_long_pressed_device._put_empty()
            self._volume_down_long_long_pressed_id = t
        t = received['playLongLongPressedId']
        if t != self._play_long_long_pressed_id:
            if self._play_long_long_pressed_id != -1: self._play_long_long_pressed_device._put_empty()
            self._play_long_long_pressed_id = t
        t = received['backLongLongPressedId']
        if t != self._back_long_long_pressed_id:
            if self._back_long_long_pressed_id != -1: self._back_long_long_pressed_device._put_empty()
            self._back_long_long_pressed_id = t
        t = received['tapId']
        if t != self._tap_id:
            if self._tap_id != -1: self._tap_device._put_empty()
            self._tap_id = t
        t = received['oidModeId']
        if t != self._oid_mode_id:
            if self._oid_mode_id != -1: self._oid_mode_device._put(received['oidMode'])
            self._oid_mode_id = t
        t = received['oidId']
        if t != self._oid_id:
            if self._oid_id != -1: self._oid_device._put(received['oid'])
            self._oid_id = t
        t = received['liftId']
        if t != self._lift_id:
            if self._lift_id != -1: self._lift_device._put(received['lift'])
            self._lift_id = t
