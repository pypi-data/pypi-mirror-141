...
class ZeroneLinkRoboid(ZeroneRoboid):
    def __init__(self, index):
        super(ZeroneLinkRoboid, self).__init__(index)
        self._motoring = {
            'module': 'zerone',
            'index': index
        }
    ...
    def _init(self, port_name=None):
        self._tag = "Zerone[{}]".format(self._index)
        Linker.register_roboid('zerone', 'zerone', self._index, self._tag, self)
        while self._ready == False and self._is_disposed() == False:
            time.sleep(0.01)

    def _release(self):
        super(ZeroneLinkRoboid, self)._release()
    ...
    def _clear_id_and_motoring(self):
        ...
        self._button_id = -1
        self._clicked_id = -1
        self._double_clicked_id = -1
        self._long_pressed_id = -1
        self._gesture_id = -1
        self._color_number_id = -1
        self._color_pattern_id = -1
        ...
        motoring['leftHeadRed'] = self._left_head_red
        motoring['leftHeadGreen'] = self._left_head_green
        motoring['leftHeadBlue'] = self._left_head_blue
        motoring['rightHeadRed'] = self._right_head_red
        motoring['rightHeadGreen'] = self._right_head_green
        motoring['rightHeadBlue'] = self._right_head_blue
        motoring['leftTailRed'] = self._left_tail_red
        motoring['leftTailGreen'] = self._left_tail_green
        motoring['leftTailBlue'] = self._left_tail_blue
        motoring['rightTailRed'] = self._right_tail_red
        motoring['rightTailGreen'] = self._right_tail_green
        motoring['rightTailBlue'] = self._right_tail_blue
    ...
    def _reset(self):
        super(ZeroneLinkRoboid, self)._reset()
        self._clear_id_and_motoring()
    ...
    def decode_sensory(self, received):
        ...
        self._front_proximity_device._put(received['frontProximity'])
        self._rear_proximity_device._put(received['rearProximity'])
        self._color_device._put_at(0, received['colorRed'])
        self._color_device._put_at(1, received['colorGreen'])
        self._color_device._put_at(2, received['colorBlue'])
        self._floor_device._put(received['floor'])
        t = received['buttonId']
        if t != self._button_id:
            if self._button_id != -1: self._button_device._put(received['button'])
            self._button_id = t
        t = received['clickedId']
        if t != self._clicked_id:
            if self._clicked_id != -1: self._clicked_device._put_empty()
            self._clicked_id = t
        t = received['doubleClickedId']
        if t != self._double_clicked_id:
            if self._double_clicked_id != -1: self._double_clicked_device._put_empty()
            self._double_clicked_id = t
        t = received['longPressedId']
        if t != self._long_pressed_id:
            if self._long_pressed_id != -1: self._long_pressed_device._put_empty()
            self._long_pressed_id = t
        t = received['gestureId']
        if t != self._gesture_id:
            if self._gesture_id != -1: self._gesture_device._put(received['gesture'])
            self._gesture_id = t
        t = received['colorNumberId']
        if t != self._color_number_id:
            if self._color_number_id != -1: self._color_number_device._put(received['colorNumber'])
            self._color_number_id = t
        t = received['colorPatternId']
        if t != self._color_pattern_id:
            if self._color_pattern_id != -1: self._color_pattern_device._put(received['colorPattern'])
            self._color_pattern_id = t
        ...
        if t != self._wheel_state_id:
            if self._wheel_state_id != -1: self._wheel_state_device._put_empty()
            self._wheel_state_id = t
        t = received['soundStateId']
        if t != self._sound_state_id:
            if self._sound_state_id != -1:
                if self._current_sound > 0:
                    if self._sound_repeat < 0:
                        self._run_sound(self._current_sound, -1)
                    elif self._sound_repeat > 1:
        ...
        if t != self._line_tracer_state_id:
            if self._line_tracer_state_id != -1: self._line_tracer_state_device._put_empty()
