...
class BaseComposite(BaseModel):
    ...
    def cached_filename(self, attr: str = "data") -> str:
        ...
        if self._cached_filenames.get(attr, None) is None:
            ...
            dirname, base = os.path.split(filebase)
            os.makedirs(dirname, exist_ok=True)
            # we retrieve all basenames starting with our base in the cache
            basenames = [
                f
            ...
            if basenames:
                # we take the last modified file
                basename = max(
                    basenames, key=lambda f: os.path.getmtime(os.path.join(dirname, f))
                )
            else:
                # else we create a new basename
                tmp_hash = MD5(f"{time.time()}{current_process().name}").hash
                basename = f"{base}-{tmp_hash}"
            self._cached_filenames[attr] = os.path.join(dirname, basename)
        ...
        for attr, loader_class in self._cached_attrs.items():
            ...
            if not os.path.isfile(filename):
                ...
                if dump_status:
                    ...
                    LOGGER.warning(
                        f"Failed to dump attribute '_{attr}' "
                        f"to tmp cached file {tmp_filename} using {loader_class}."
                    )
