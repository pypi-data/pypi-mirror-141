...
class Rules(CommonRules):
    ...
    def _create_df(
        ...
        raw_df = self._get_df(file_kind=file_kind, index_col=0).dropna(
            axis=1, how="all"
        )
    ...
    def preprocessed_files_df(self) -> pd.DataFrame:
        return self._create_df("preprocessed_files", concat_axis=0, nb_days=2)
    ...
    def get_file_info(self, file_id: str) -> pd.Series:
        ...
        step = Timedelta(hours=int(file_info["step"]))
        term_range = range(int((file_info["stop"] - file_info["start"]) / step) + 1)
        file_info.loc["terms"] = [
            Datetime(file_info["start"] + step * i) for i in term_range
        ]
        return file_info
    ...
    def best_preprocessed_file_term(
        ...
        """
        for file_id in self.preprocessed_files_df.index:
            file_info = self.get_file_info(file_id)
            if term not in file_info.get("terms", []):
                continue
            if file_info.get("geometry", "") not in geometries:
                continue
            params_ok = True
            for param in params:
                full_root_param, _ = self.param_to_description(param)
                if full_root_param not in self.files_links_df[file_id].dropna().index:
                    params_ok = False
                    break
            if params_ok:
                return [file_id] + file_info[["date", "stop"]].to_list()
        return None
    ...
    def best_preprocessed_files(
        ...
        if start > stop or self.bulletin_datetime > stop:
            return []
        correct_start = max(self.bulletin_datetime, start)
        ...
        if file_stop >= stop:
            ...
            if file_stop < self.bulletin_datetime.midnight + Timedelta(days=2):
                new_start = file_stop + Timedelta(hours=1)
            else:
                new_start = file_stop + Timedelta(hours=3)

            other_files = self.best_preprocessed_files(
                new_start, stop, geometries, params
            )

        start_time = str(correct_start)
        stop_time = str(file_stop)
        return [(filename, start_time, stop_time)] + other_files
