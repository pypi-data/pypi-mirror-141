...
def compute_sum_futur(
    ...
    # On merge
    dout = xr.merge([RR6_beg, RR6_end])
    # Maintenant on rempli des infos dans les attributs.
    dout[da.name].attrs["accum_hour"] = stepout
    dout[da.name].attrs["accum_type"] = "futur"
    if "GRIB_startStep" in dout[da.name].attrs:
        dout[da.name].attrs["GRIB_endStep"] = (
            dout[da.name].attrs["GRIB_startStep"] + stepout
        )
        dout[da.name].attrs["GRIB_stepRange"] = "{}-{}".format(
            dout[da.name].attrs["GRIB_startStep"], dout[da.name].attrs["GRIB_endStep"]
        )
    return dout
