...
class EnvOpenx():
    ...
    def init(self, path):
        ...
        ego_v,ego_x,ego_y = [float(i.split('=')[1]) for i in ego_init.split(',')]
        # 获取行驶目标
        ...
        flag = -1 if ego_v < 0 else 0
        self.vehicle_array[0,:] = [ego_x,ego_y,np.abs(ego_v),flag*np.radians(180),self.car_length,self.car_width]
        # 将所有其他车辆设置到0时刻位置
        i = 1
        ...
        for vehi in self.traj.keys():
            if self.t in self.traj[vehi].keys():
                ...
                self.vehicle_array[i,3] = flag*np.radians(180) # 转向角 没有数值
            else:
