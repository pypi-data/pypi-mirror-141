...
class AutoCrosswalk(object):
    ...
    def generate_crosswalk(self,
        ...
        # ---------------------------------------------------------------------
        self.transition_matrix = self.estimate_transition_matrix(df_from=df_from,
                                                            df_to=df_to,
        ...
        if self.prioritize_exact_match:
            # Find exact matches
            crosswalk_exact = self._find_exact_match(transition_matrix=self.transition_matrix,
        ...
        mask_lookup = self.transition_matrix["average"].index.isin(crosswalk.loc[mask_find].index)

        if mask_lookup.sum()>0:
            # Find nbest candidates
            crosswalk_nbest = self._find_nbest_match(transition_matrix=self.transition_matrix["average"].loc[mask_lookup],
                                                     n=self.n_best_match)
