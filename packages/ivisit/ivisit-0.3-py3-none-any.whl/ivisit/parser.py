...
class IVisitParser:
    ...
    def parse_simulation(self,lines_of_simulation, linenumbers_of_simulation):
        ...
        for i in range(len(lines_of_simulation)):
            ...
            if terms[0]=="SIMULATION":
                ...
                assert nterms==4, str_parseerror+"Syntax Error in TEXT_OUT DIRECTIVE, nterms="+str(nterms)+"! "\
                    +"Instead please use syntax @IVISIT:TEXT_OUT & <textoutbox_name> & <size> & <dataname>"
                self.new_textout(terms[1],terms[2],terms[3])
            else:
    ...
    def new_textout(self,txo_name,txo_size,txo_dataname):
        ...
        if self.db!=None:
            ...
            if len(data)<=0: # no data widget available --> insert new one in table 
                ...
                self.db.simple_insert('datawidget',\
                                      ['key'      ,'key_simulation','key_dataarray','name'  ,'type'     ,'range','scale'],\
                                      [datawdg_key,self.simkey     ,datakey        ,txo_name,'textfield','None' ,1.0])   # insert new data widget entry
            else:
                datawdg_key=data[0][0]
                if self.parsemode in [self.PARSEMODE_UpdateOld,self.PARSEMODE_UpdateOldExceptValues]:
                    self.db.simple_update_byPKEY('datawidget',\
                                                 ['type'     ,'range','scale'],\
                                                 ['textfield','None' ,1.0    ], ['key'],[datawdg_key])   # update data widget entry
