...
class AndTerminator(Terminator):
    """
    ...
    def should_terminate(self, population: SampleSet) -> bool:
        return all(s.should_terminate(population) for s in self.stoppers)


class AndTermination(Termination):
    """
    A factory that produces instances `AndTerminator`.
    """
    def __init__(self, stops: List[Termination]):
        self.stops = stops

    def initialise(self, problem: BinaryQuadraticModel) -> Terminator:
        return AndTerminator([s.initialise(problem) for s in self.stops])
