...
def main_loop(window: sg.Window, foliation: fo.Foliation):
    ...
    while True:
        # print(type(window['paschal']))
        # print(type(window['table_of_contents']))
        # print(type(window['papyrus']))
        # print(type(window['width_min']))
        # print(type(window['exterior']))
        # print(type(window['date_range_start']))
        # if isinstance(window['paschal'], sg.Checkbox):
        #     print("it's a check")
        
        if load_on_start:
            foliation = rdf.load_from_saved(window, load_on_start)
            window.set_title(f"Prepify v{__VERSION}: {Path(args[1]).name}")
        ...
        if event in [sg.WIN_CLOSED, None, sg.WIN_X_EVENT]:
            ...
            if answer == 'OK':
                foliation = rdf.load_from_saved(window, 'prepify/resources/new.csntm', store_saved_path=False)
                saved = False
...
def main():
    ...
    try:
        ...
        # try:
        values = window.read(timeout=0)
        sg.popup_ok(f'Prepify crashed for a reason unknown to DF.\nYou will be prompted to save the prepdoc in progress as a last resort.\nIt is unlikely that your data was lost.\n(For David: {e})', title='BUMMER')
        save_path = rdf.save_doc_data(values, window, foliation)
        # except Exception as e:
