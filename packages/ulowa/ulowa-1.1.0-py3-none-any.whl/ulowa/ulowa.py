...
def ulowa(problem_labels, weights_ulowa, fuzzyNumbers_ulowa, labels_ulowa):
    """
    ...
    if (len(problem_labels) >= 2):
        ...
        return ulowa(problem_labels, weights_ulowa, fuzzyNumbers_ulowa, labels_ulowa)
...
def plotFuzzySets(fuzzySets, labels):
    ...
    f.set_figheight(6)
    # plotting the points
    for i in range(0, len(fuzzySets)):
        plt.plot(fuzzySets[i], y[i], label=labels[i])
    plt.legend()
