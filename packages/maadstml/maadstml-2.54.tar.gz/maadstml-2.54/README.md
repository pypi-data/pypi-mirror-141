...
- **viperpreprocessproducetotopicstream**
  - Users can pre-process data streams using the following functions: MIN, MAX, AVG, COUNT, DIFF, SUM, MEDIAN, VARIANCE, OUTLIERS, ANOMPROB.  
...
**4. maadstml.viperproducetotopic(vipertoken,host,port,topic,producerid,enabletls=0,delay=100,inputdata='',maadsalgokey='',
	maadstoken='',getoptimal=0,externalprediction='',subtopics='',topicid=-999,identifier='',dependentvariable='',brokerhost='',
...
- You can stream multiple variables at once, and use dependentvariable to specify which variable is the dependent variable.
  ...
  dependentvariable="B", then when you do machine learning on this data, the variables A, B, C are date/time synchronized
  and stream B will be automatically treated as the dependentvariable, and streams A and C as independentvariables.
...
- If you only want to search for record that have a particular processtype, you can enter:
  MIN, MAX, AVG, COUNT, DIFF, SUM, MEDIAN, VARIANCE, OUTLIERS, ANOMPROB.   ANOMPROB=Anomaly probability,
...
- You can set conditions to aggregate functions: MIN, MAX, AVG, COUNT, DIFF, SUM, MEDIAN, VARIANCE, OUTLIERS, ANOMPROB.
...
- Here you need to specify how you want to pre-process the streams.  You can perform the following operations:
  MAX, MIN, AVG, COUNT, SUM, DIFF, VARIANCE, MEDIAN, OUTLIERS, ANOMPROB.  The order of the operation must match the 
