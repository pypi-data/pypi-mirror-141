...
class MultiWindowGenerator(Component):
    ...
    def train(self):
        ...
        if not hasattr(self,"train_values"):
            train_values=self._train_df.groupby(["id", "id_sort"])

            self.train_values=self.split_window(np.array([train_values.get_group(group).drop(columns=["id", "id_sort", "id_group"]) for group in train_values.groups]))
    ...
    def val(self):
        ...
        if not hasattr(self, "val_values"):
            ...
            val_values=self._valid_df.groupby(["id", "id_sort"])

            self.val_values=self.split_window(np.array([val_values.get_group(group).drop(columns=["id", "id_sort", "id_group"]) for group in val_values.groups]))
    ...
    def test(self):
        if not hasattr(self, "test_values"):
            ...
            test_values=self._test_df.groupby(["id", "id_sort"])
            test_values=np.array([test_values.get_group(group).drop(columns=["id", "id_sort", "id_group"]) for group in test_values.groups])
            self.test_values=self.split_window(test_values)
    ...
    def test_df(self):
        if hasattr(self, "test_df_"):
            ...
            test_values = self._test_df.groupby(["id", "id_sort"])
            test_values = np.array([test_values.get_group(group).drop(columns=["id"]) for group in test_values.groups])
