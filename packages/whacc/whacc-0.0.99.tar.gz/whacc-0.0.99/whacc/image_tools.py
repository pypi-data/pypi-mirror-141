...
def combine_and_index_feature_data(h5s_to_combine, new_h5, label_key='labels', feature_key='FD__original'):
    h5c = image_tools.h5_iterative_creator(new_h5,
                                           overwrite_if_file_exists=True,
                                           max_img_height=1,
                                           max_img_width=2048,
                                           close_and_open_on_each_iteration=True,
    ...
    for i, k in enumerate(h5s_to_combine):
...
def check_if_permuted_h5_matches_og_h5_indexes(h5_1, h5_2, ind):
    with h5py.File(h5_1, 'r') as h:
        with h5py.File(h5_2, 'r') as hperm:
            i1 = np.where(h['h5_inds_all'][:] == ind)[0]
            i2 = np.where(hperm['h5_inds_all'][:] == ind)[0]
            ...
            x1 = hperm['inds_to_h5_file_names'][i2]
            print(hperm['all_combined_h5_names'][x1])
            print(hperm['h5_inds_all'][i2])
            print(hperm['h5_inds_each'][i2])
            print(x1)
            print(hperm['FD__original'][i2, :])
