...
class ConfusionMatrixWidget:
    ...
    def __acc_score_equation(self, w):
        with self.accuracy_plt:
            ...
            tp, fp, fn, tn = self._cnf_matrix.ravel()
            ...
            """
            temp = Template(s).substitute(TP=tp, FP=fp, TN=tn, FN=fn, SCORE=score)
            display(Math(temp))
    ...
    def __precision_score_equation(self, w):
        with self.precision_plt:
            ...
            self.__generate_cnf(self.t_widget.value)
            tp, fp, _, _ = self._cnf_matrix.ravel()
    ...
    def __recall_score_equation(self, w):
        with self.recall_plt:
            ...
            self.__generate_cnf(self.t_widget.value)
            tp, _, fn, _ = self._cnf_matrix.ravel()
    ...
    def __cnf_matrix(self, w):
        ...
        with self.cnf_plot:
            ...
            for i, j in itertools.product(
                range(self._cnf_matrix.shape[0]), range(self._cnf_matrix.shape[1])
            ):
                # Add patch
                sq = Rectangle(
                    xy=(i, j),
    ...
    def show(self):
        ...
        self.accuracy_plt = widgets.Output(
            layout=widgets.Layout(height="80px", width="300px")
        )
