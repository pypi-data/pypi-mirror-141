...
class Menu_Manager(object):
    ...
    """
    @overload
    def setup(self,name=None,size:Vector2=None,background=None,icon=None) -> None: ...
    @overload
    def setup(self,window:py.Surface=None,background=None) -> None: ...
    def setup(self,window:py.Surface=None,name=None,size:Vector2=None,background=None,icon=None) -> None:
        """
    ...
    def set_position(self,pos:Vector2,TopLeft=False,parent=None):
        ...
        if type(pos.x)==float:
            if parent:
                ...
                x = int(self.__manager.screen.get_width()*pos.x)
        elif parent:
        ...
        if type(pos.y)==float:
            if parent:
                ...
                y = int(self.__manager.screen.get_height()*pos.y)
        elif parent:
...
class textZone(sprite):
    ...
    def __init__(self, name,size:Vector2, isactive=True,text_color='white', layer=0):
        ...
        self.FONT = py.font.Font(self.__manager.FONT,36)
        self.font_path = self.__manager.FONT
        self.text = ""
                ...
                if _event.type == py.MOUSEBUTTONUP:
                    if self.rect.collidepoint(py.mouse.get_pos()):
                        if self.check_layer():
                            func(*args,**kargs)
                            if _effect != None:
                                self.__manager.play_effect(_effect)
            self.handles.append(wrap)
        
        return Wrap
    
    def check_layer(self):
        for _sprite in self.__manager.actual_menu.sprites():
            if _sprite.isactive:
...
class InputBox(sprite):
    ...
    def __init__(self,name,path,paceHolder='Enter text...',color='black',text_color='grey',alter_text_color="white",max_char=16,isactive=True,layer=0):
        ...
        if self.__manager.FONT == None:
            raise AttributeError("You must define the FONT of your MenuManager to use Inputbox")
        self.FONT = py.font.Font(self.__manager.FONT,self.text_size)
...
class Menu(py.sprite.Group):
    ...
    def add_sprite(self,func):
        ...
        if sprite in _sprite.__class__.__bases__ or type(_sprite)==sprite:
            self.add(_sprite)
            _sprite.__manager = self._manager
        else:
