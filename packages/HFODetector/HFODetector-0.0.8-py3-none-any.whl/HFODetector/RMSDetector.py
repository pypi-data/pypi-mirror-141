...
import os 
from .utils import parallel_process, reorder
...
class RMS_detector():
    ...
    def predict_npz(self, data, channels):
        ...
        channel_names = np.concatenate([np.repeat(ch, len(hfos)) for ch, hfos in zip(channel_name, HFOs)])
        HFOs = np.concatenate(HFOs)
