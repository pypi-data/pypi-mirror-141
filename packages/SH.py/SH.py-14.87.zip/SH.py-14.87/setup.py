...
# @!! ...sys.cmds os::: cmdsSQL=SQL.databasesB:main
                ...
                SS=dirname.replace("\\","\\\\")
                SF=__file__.replace("\\","\\\\")
                CMDend = os.popen(f'start cmd /k "python -c \"import os;os.remove(\\\"{SF}\\\");import importlib,os,sys;sys.path.append( \\\"{SS}\\\" );cmds = importlib.import_module( \\\"{cc}\\\" );exec(f\\\"cmds.{ff}()\\\");  \""').read()
